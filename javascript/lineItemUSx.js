...
//alert("FAcotry rep");
	...
	function updatePriceCalc(searchXML){
		...
		for(var i=0;i<count;i++){
			...
			if((document.form1.product.value=="GCP"||document.form1.product.value=="IWP"||document.form1.product.value=="EJC"||document.form1.product.value=="EFS")&&document.form1.group.value.indexOf("REP")<0){
				...
				if(TEMPfreightOverride=="Y"){
					...
					if(document.form1.product.value=="EJC"||document.form1.product.value=="GCP"){
						document.priceCalcForm.handlingCost.disabled=true;
...
//alert("is sequence over 1000 and issfdc length==0");
			...
			if(searchXML.getElementsByTagName("action")[i].childNodes[0].nodeValue=="QUOTE OVER PRICE LIMIT"){
				...
				if(searchXML.getElementsByTagName("name")[i].childNodes[0].nodeValue=="Rep Portal OWS"){
					newOwsUrl="https://sfxtdev.c-sgroup.com:8443/ows_webapp/OWS/owsApplication/orderWriteUpSheet.html#/ows/"+document.form1.orderNo.value+"/US_SANDBOX/"+document.form1.userId.value+"/"+document.form1.product.value;
	...
	function saveHeader(){
		...
		for(var i=0;i<fieldRequired.length;i++){
			if(!(fieldRequired[i]=="quoteSource"&&(document.form1.qtype.value=="alt"||(document.form1.orderNo.value.indexOf("_00")<1&&document.form1.orderNo.value.length>0)))){
				if(!(document.form1.creatorGroup.value=="Decolink"&&(fieldRequired[i]=="custLoc"||fieldRequired[i]=="custName"))){
...
//alert("a");
		if(alertMsg.length==l_Msg&&winLossDesc.length==0){
			...
			params=params+"&archName="+document.headerForm.archName.value;
			params=params+"&archLoc="+document.headerForm.archLoc.value;
...
//htmlLines=htmlLines+cseUrl+"<BR><BR>";
			if(TEMPlineNo*1>lastLineNo*1){
				...
				if((document.form1.group.value.indexOf("Rep")>=0&&document.form1.product.value=="GE")||document.form1.group.value.indexOf("Rep")>=0&&(document.form1.projectType.value=="SFDC"||document.form1.projectType.value=="PSA")&&(quoteType=="Q"||TEMPproductId=="ADS"||document.form1.product.value=="GE"||TEMPproductId=="LVR"||TEMPproductId=="GRILLE")){
