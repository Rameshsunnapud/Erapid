...
//out.println(group_id+"::<BR>");
		...
		if(group_id.equals("Admins")||group_id.equals("admins")||group_id.equals("develop") || group_id.equals("Develop")||group_id.equals("super")){
			out.println("<form name='showx' action='qualifying_notes_maint.jsp'>");
        ...
        else if(userName.toLowerCase().equals("dchapman")){
			out.println("<form name='showx' action='qualifying_notes_maint.jsp'>");
			out.println("PRODUCT ID:<select name='sbu' onchange='document.showx.submit();'>");
			out.println("<option value=''></option>");
			out.println("<option value='EJC'>EJC</option>");
			out.println("<option value='CPS'>CPS</option>");
			out.println("</select></form>");
		}else if(group_id.toUpperCase().equals("MUN-ADMIN")){
			out.println("<form name='showx' action='qualifying_notes_maint.jsp'>");
			out.println("PRODUCT ID:<select name='sbu' onchange='document.showx.submit();'>");
			out.println("<option value=''></option>");
			out.println("<option value='ADS'>ADS</option>");
            out.println("<option value='EFS'>EFS</option>");
			out.println("<option value='EJC'>EJC</option>");
			out.println("<option value='IWP'>IWP</option>");
			out.println("</select></form>");
		}
		...
		if(operation == null || operation.trim().length()<1 || operation.equals("show")){
			...
			//out.println("select * from cs_qlf_notes where product_id = '"+sbu+"' order by code, comp_code, cast(description as varchar)");
			ResultSet rs1=stmt.executeQuery("select * from cs_qlf_notes where product_id = '"+sbu+"' order by code, comp_code, cast(description as varchar)" );
			if(rs1 != null){
				while(rs1.next()){
					...
					if(rs1.getString("model") != null&&rs1.getString("model").trim().length()>0){
			...
			rs1.close();
			out.println("<form name='show' action='qualifying_notes_maint.jsp'>");
			...
			out.println("<tr><td width='5%'>Comp Code</td><td width='5%'>Code</td><td width='65%'>Description</td>");
			if(!sbu.equals("LVR")){
				out.println("<td width='15%'>Model</td>");
			}
			out.println("<td width='5%'>Effective Date</td>");
			out.println("<td width='5%'>Default</td>");
			if(sbu.equals("ADS")){
				out.println("<td width='15%'>Type</td>");
			}
			out.println("<td width='5%'>Edit</td></tr>");
			for(int a=0; a<comp_code.size(); a++){
				out.println("<tr>");
			...
			rs1.close();
			out.println("<form name='editThis' action='qualifying_notes_maint.jsp'>");
			out.println("<input type='hidden' name='sbu' value='"+sbu+"'>");
			out.println("<input type='hidden' name='operation' value=''>");
			out.println("<input type='hidden' name='recordNum' value=''>");
			out.println("<table border='1' width='100%'>");
			...
			out.println("<td width='5'>Effective Date</td><td width='5'>Default</td><td width='5%'>Save</td></tr>");
			out.println("<tr>");
			...
			String ccode="";
			if(sbu.equals("LVR")|sbu.equals("GRILLE")){
				ccode="A3";
			}
			if(sbu.equals("GE")){
				ccode="GE";
			...
			out.println("<form name='addThis' action='qualifying_notes_maint.jsp'>");
			out.println("<input type='hidden' name='sbu' value='"+sbu+"'>");
			out.println("<input type='hidden' name='operation' value=''>");
			if(!sbu.equals("ADS")){
				out.println("<input type='hidden' name='type' value=''>");
			...
			out.println("<table border='1' width='100%'>");
			out.println("<tr><td width='5%'>Comp Code</td><td width='5%'>Code</td><td width='85%'>Description</td>");
			if(!sbu.equals("LVR")){
				out.println("<td width='15%'>Model</td>");
			}
			if(sbu.equals("ADS")){
				out.println("<td width='5%'>Default</td><td>Type</td><td width='5%'>Save</td></tr>");
			}
			else{
			...
			out.println("<tr>");
			out.println("<td><input type='text' name='compCode' value='"+ccode+"' readonly></td>");
			...
			try{
				java.sql.PreparedStatement update_qlfNotes = myConn.prepareStatement("UPDATE cs_qlf_notes set description= ?,model=?,set_default=?,type=?,effective_date=? where product_id= ? and comp_code=? and code=?  ");
				update_qlfNotes.setString(1,description);
				update_qlfNotes.setString(2,model);
				update_qlfNotes.setString(3,set_default);
				update_qlfNotes.setString(4,type);
				update_qlfNotes.setString(5,effectiveDate);
				update_qlfNotes.setString(6,sbu);
				update_qlfNotes.setString(7,compCode);
				update_qlfNotes.setString(8,code);
				int roCount=update_qlfNotes.executeUpdate();
			}
			catch (java.sql.SQLException e)
			{
				out.println("Problem with updating cs_qlf_notes table"+"<br>");
				out.println("Illegal Operation try again/Report Error"+"<br>");
				myConn.rollback();
				out.println(e.toString());
				return;
			}
			out.println("<form name='editDone' action='qualifying_notes_maint.jsp'>");
			out.println("<input type='hidden' name='sbu' value='"+sbu+"'>");
			out.println("<input type='hidden' name='operation' value=''>");
			out.println("<input type='hidden' name='recordNum' value=''>");
			out.println("<table border='1' width='100%'>");
			...
			if(sbu.equals("ADS")){
				out.println("<td widht='5%'>Effective Date</td><td widht='5%'>Default</td><td>Type</td><td width='5%'>Continue</td></tr>");
			}
			else{
				out.println("<td widht='5%'>Effective Date</td><td widht='5%'>Default</td><td width='5%'>Continue</td></tr>");
			}
			out.println("<tr>");
			...
			if(sbu.equals("ADS")){
				out.println("<td>"+set_default+"</td><td>"+type+"</td><td><input type='button' name='continue' value='Continue' onclick='goBack1()'></td>");
			}
			else{
			...
			if(!isGood){
				...
				int rowCount=0;
				ResultSet rs1=stmt.executeQuery("select count(*) from cs_qlf_notes where product_id='"+sbu+"' and comp_code='"+compCode+"' and code='"+code+"'");
				if(rs1 != null){
					while(rs1.next()){
				...
				if(rowCount>0){
					...
					out.println("<td width='5%'>Save</td></tr>");
					out.println("<tr>");
					out.println("<td><input type='text' name='compCode' value='"+compCode+"' ></td>");
					out.println("<td><input type='text' name='code' value='"+code+"'></td>");
					out.println("<td><textarea name='description' rows='3' cols='100'>"+description+"</textarea></td>");
					...
					try{
						String insert ="INSERT INTO cs_qlf_notes(comp_code,code,product_id,description,model,set_default,type,effective_date)VALUES(?,?,?,?,?,?,?,?) ";
						PreparedStatement update_qlf_notes = myConn.prepareStatement(insert);
						update_qlf_notes.setString(1,compCode.toUpperCase());
						update_qlf_notes.setString(2,code);
						update_qlf_notes.setString(3,sbu.toUpperCase());
						update_qlf_notes.setString(4,description);
						update_qlf_notes.setString(5,model);
						update_qlf_notes.setString(6,set_default);
						update_qlf_notes.setString(7,type);
						update_qlf_notes.setString(8,effectiveDate);
						...
						out.println("Problem with ENTERING TO cs_qlf_notes TABLEs"+"<br>");
						out.println("Illegal Operation try again/Report Error"+"<br>");
						myConn.rollback();
						out.println(e.toString());
						return;
