...
%>
<TABLE cellspacing=0 cellpadding=2 id=header width='100%' border='1'>
	<tr>
	...
	ResultSet rsUsedModels=stmt.executeQuery("Select * from cs_ic_price order by sbu,model");
...
if(usedItems.trim().length()>0){
	...
	<TABLE cellspacing=0 cellpadding=2 id=header width='100%' border='1'>
		...
		rs0x.close();
		String query2x="Select PRCITM,PRCCST,PRCSBU,PRCSUM,PRCWGT,PRCBAL,PRCDSC,PRCDS2 from CS_PRC001PF order by PRCSBU,PRCITM";
		//out.println(query2x);
		int count2=0;
				...
				for(int a=1; a<=8; a++){
					...
					if(rsUsedItems.getString(a) != null && rsUsedItems.getString(a).trim().length()>0){
						...
						if(a==2){
							...
							out.println("<td "+align+" "+wth+">"+rsUsedItems.getString(a).trim()+"</td>");
						}
					}
					else{
						out.println("<td>&nbsp;</td>");
