...
String fax="";
if(customer.startsWith("US")){
	customer=customer.substring(2);
}
if(customer != null && customer.length()>2 && (project_type ==null || project_type.trim().length()==0)){
ResultSet rs_cust=stmt.executeQuery("select * from cs_customers where cust_no ='"+customer+"'");
if(rs_cust != null){
	while(rs_cust.next()){
out.println(rs_cust.getString("cust_name1")+"<BR>");
...
for1.setMaximumFractionDigits(0);
titl="Packing List";
int count=0;Vector block_code=new Vector();Vector detail_desc=new Vector();Vector qty_ordered=new Vector();Vector item_no=new Vector();
Vector line_no_group=new Vector();Vector tot_group=new Vector();int count_group=0;int count_mat=0;Vector seq_no=new Vector();
...
if (ii<=0){out.println("<p class='pbr'>&nbsp;</p>");}
	...
	*/
	...
	out.println("</form></body></html>");
		...
		URL output_st = new URL("https://"+application.getInitParameter("HOST")+"/custom/orders/efs/shop.transfer.jsp?sp-q="+order_no+"&cmd=1");
		URL output_st1 = new URL("https://"+application.getInitParameter("HOST")+"/custom/orders/efs/shop.transfer.jsp?sp-q="+order_no+"&cmd=2");
		URL output_st2 = new URL("https://"+application.getInitParameter("HOST")+"/custom/orders/efs/shop.transfer.jsp?sp-q="+order_no+"&cmd=3");
		URL output_st3 = new URL("https://"+application.getInitParameter("HOST")+"/custom/orders/efs/shop.transfer.jsp?sp-q="+order_no+"&cmd=4");
		URL output_st10 = new URL("https://"+application.getInitParameter("HOST")+"/custom/orders/efs/shop.transfer.jsp?sp-q="+order_no+"&cmd=10");
