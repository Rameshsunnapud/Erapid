...
//									t3=t3+tv;									
								...
								//
//								final_out="";
								final_out=final_out+county+"MU"+t3+t1+t4+"0000"+"  "+"N"+"2 "+shop_order_no.elementAt(i)+"\r\n";
...
//								 out.write("out put done "+"<br>");
				  ...
				  if( (product_desc.elementAt(i).toString().equals("pan")) ){
						ResultSet rs_efs_shop_paper1 = stmt.executeQuery("SELECT part_no,cast(detail_desc as varchar(600)),bpcs_part_sub,cast(sum(qty)as integer) FROM cs_materials_output where order_no like '"+order_no+"' and item_no='"+shop_item_no.elementAt(i)+"' and text_identifier like 'PAN%' group by part_no,cast(detail_desc as varchar(600)),bpcs_part_sub");
							if (rs_efs_shop_paper1 !=null) {
								while (rs_efs_shop_paper1.next()){
								String t1=rs_efs_shop_paper1.getString(1);																
								String t2=rs_efs_shop_paper1.getString(2);
...
//								 out.write("out put done "+"<br>");
				  ...
				  if( (product_desc.elementAt(i).toString().equals("frame")) ){
						ResultSet rs_efs_shop_paper1 = stmt.executeQuery("SELECT bpcs_part_sub,PART_NO,cast(sum(qty)as integer) FROM cs_materials_output where order_no like '"+order_no+"' and text_identifier like 'FRAME%' AND bpcs_part_sub='"+part_sub.elementAt(i)+"' group by bpcs_part_sub,part_no ");
							if (rs_efs_shop_paper1 !=null) {
								while (rs_efs_shop_paper1.next()){
...
//									        out.println("N"+k1+k2_notes+k3+k4+notes_4+notes_5+notes_6+notes_7+"MU   "+"<br>");
											final_notes_out=final_notes_out+"N"+k1+k2_notes+k3+k4+notes_4+notes_5+notes_6+notes_7+"MU   "+"\r\n";
										 }												
									}//while
					   ...
					   final_out=final_out+"R"+k1+k2+k3+k4+k5+k6+" "+k7+k8+"MU"+"1  "+"\r\n";
					   count_rout_rows++;
