...
<html>
	...
	<SCRIPT language="JavaScript">
		function poponload(){
			...
			var props='scrollBars=yes,resizable=yes,toolbar=yes,menubar=no,location=no,directories=no,width=450,height=200,status=yes';
	...
	<body onload="poponload()">
		...
		<%@ page language="java" import="javax.mail.*" import="java.sql.*" import="java.util.*" import="java.lang.*" import="java.io.*" import="java.text.*" import="javax.mail.internet.*" import="javax.activation.*" import="java.net.*" import="java.math.*" errorPage="error.jsp" %>
...
String order_type="";
	...
	String userId=request.getParameter("userId");
		...
		String dir_path1="D:\\TRANSFER\\BPCS_OE\\testing\\ADS\\";
		...
		if (rs_find !=null) {
			while (rs_find.next()){
				...
				section_transfer=rs_find.getString("section_transfer");
			}
		}
		rs_find.close();
					if(section_transfer==null){
						section_transfer="";
					}
					if(section_transfer.trim().length()>0){
						section_transfer="Y";
					}
		// Project info
		...
		if (rs_project !=null) {
			while (rs_project.next()) {
				...
				//out.println(bpcs_tier_order+"::<BR>");
		...
		if(quote_origin.equals("Alternate")||quote_origin.equals("")||quote_origin.equals("Copy")||quote_origin.equals("As Build")){
			base_quote_no=order_no;
		}
		if(handling_cost != null){
			handling_cost=handling_cost.replaceAll(",","");
		}
		if(setup_cost != null){
			setup_cost=setup_cost.replaceAll(",","");
		}
		if(overage!= null){
				overage=overage.replaceAll(",","");
		}
		if(freight_cost != null){
			freight_cost=freight_cost.replaceAll(",","");
		}
		//out.println(base_quote_no+":: 1base quote number<BR>");
		if(base_quote_no.trim().length()>0 && base_quote_no.startsWith(order_no.substring(0,6)) && quote_origin.trim().length()>0 && !quote_origin.equals("Alternate") &&!(quote_origin.equals("As Build") &&bpcs_order_no.trim().length()==0 )){
			...
			//out.println(base_quote_no+"::2 base quote number<BR>");
		}
			...
			//out.println(base_quote_no+":: 3base quote number<BR>");
		}
		//out.println(base_quote_no+":: 4base quote number<BR>");
		...
		String largestLine="";double largestPrice=0.0;int counterLarge=0;int indexLarge=-1;Vector add_release_flag=new Vector();
Vector bpcsNote=new Vector();
Vector bpcsNoteLineNo=new Vector();
		int diLineCounter=0;double nonCommission=0;
		String sql_one="";String str="SELECT * FROM csquotes where order_no like '"+order_no+"' and line_no in ("+lines_final+") and (block_id like 'a_%' or block_id like '%_notes') order by cast(Line_no as integer)";
		//out.println(lines_final+":: HERE");
		...
		while ( rs3.next() ) {
			...
			//out.println(rs3.getString("line_no")+"::"+rs3.getString("block_id")+"::<BR>");
			if(rs3.getString("block_id").toUpperCase().startsWith("A")){
				...
				if(rs3.getString("bpcs_tier") != null && rs3.getString("bpcs_tier").trim().length()>0){
					...
					bpcs_tier_lines.addElement("0");
				}
				//out.println("::"+quote_origin+"::"+rs3.getString("deduct")+"::<BR>");
				if(quote_origin.startsWith("change")&& rs3.getString("deduct")!=null && rs3.getString("deduct").trim().equals("add")){
				...
				if(rs3.getString("bpcs_note")!=null &&rs3.getString("bpcs_note").trim().length()>0 && rs3.getString("block_id").toUpperCase().equals("A_APRODUCT")){
					bpcsNote.addElement(rs3.getString("bpcs_note").trim());
					bpcsNoteLineNo.addElement(rs3.getString("line_no").trim());
					//out.println(rs3.getString("bpcs_note").trim()+"::"+rs3.getString("line_no")+"<BR>");
				}
				//add_release_flag.addElement("");
				counterLarge++;
			}
			else{
                                if(status.equals("RF")&&rs3.getString("descript")!=null &&rs3.getString("descript").trim().length()>0){
                                            bpcsNote.addElement(rs3.getString("descript").trim());
                                            bpcsNoteLineNo.addElement(rs3.getString("line_no").trim());
                                }
			}
		}
		...
		*/
	 	class DNOTE{
			...
			private String notesLineNumber;
			
			public String getNotesLineNumber(){
				return notesLineNumber;
			}
			public void setNotesLineNumber(String notesLineNumber){
				this.notesLineNumber=notesLineNumber;
			}
		...
		String queryString = "SELECT Descript, Block_ID, Line_no FROM csquotes WHERE Order_no like"+ 
				"'"+order_no+"' AND Product_ID = 'ADS' AND Block_ID IN ('D_NOTES', 'DA_NOTES', 'DI_NOTES')";
		//out.println("Query for Active and D-Active notes is : "+queryString);
		ResultSet dNotesRs =  stmt.executeQuery(queryString);
		List<DNOTE> dNotes = new ArrayList<DNOTE>();
		
		if (dNotesRs !=null) {
			while (dNotesRs.next()) {
				
			 String strDescript = dNotesRs.getString("Descript");
			 String strBlockID = dNotesRs.getString("Block_ID");
			 String notesLINumber = dNotesRs.getString("Line_no");
			 DNOTE dNote = new DNOTE();
			 dNote.setDescription(strDescript);
			 if(strBlockID.equalsIgnoreCase("DA_NOTES")){
				 strBlockID = "Active";
			 }
			 else if(strBlockID.equalsIgnoreCase("DI_NOTES")){
				  strBlockID = "InActive";
			 }
			 dNote.setBlockID(strBlockID);
			 dNote.setNotesLineNumber(notesLINumber);
			 dNotes.add(dNote);
			}
		}
...
order_type=cs_order_type;
		//out.println(cs_order_type+":: cs order type");
		...
		String ship_prior_notice_name="";String ship_prior_notice_phone="";String ship_prior_notice="";String shipping_bpcs_no="";
		...
		if (rs_ship !=null) {
			while (rs_ship.next()) {
				...
				shipping_bpcs_no=rs_ship.getString("bpcs_cust_no");
			   }
		}
		//restting nulls
		...
		if(ship_to_customer==null ||ship_to_customer.trim().length()==0||ship_to_customer.trim().equals("0")){
			if(	shipping_bpcs_no != null && shipping_bpcs_no.trim().length()>0){
				ship_to_customer=shipping_bpcs_no;
			}
		}
...
con_bpcsusrmm.close();
		...
		if(customer_po_no==null){customer_po_no=" ";}if(payment_terms.startsWith("NET")){payment_terms="C";}else{payment_terms="V";}if(Project_name==null){Project_name=" ";}
		...
		//out.println("::"+customer_po_no+"::2<BR>");
		...
		//market ends
		//2.Region starts
		sales_region=sales_region.substring(0,2);
		...
		//out.println(win_loss);
			...
			if(product_id.equals("ADS")){
				...
				String line_no_text="";String blockId="";String ads_bpcs_item="";String ads_qty="";double ads_qty_up=0; double ads_price=0;double ads_com_price=0;
				String bpcs_tier_line="";
				//Vars
				//out.println("<table border='1' width='100%'><tr><td>order no</td><td>line no</td><td> mark</td><td> part number</td><td> qty</td><td> base cost</td><td>unit cost</td><Td>total cost</td><td>unit sell</td><td> total sell</td></tr>");
				for(int ii=0;ii<line_item.size();ii++){
					//out.println(line_item.elementAt(ii).toString()+block_id.elementAt(ii).toString()+":x:<BR>");
					if(!block_id.elementAt(ii).toString().equals("A_APRODUCT")){
//out.println("select * from cs_ads_order_entry where quote_no='"+order_no+"' and line_no='"+line_item.elementAt(ii).toString()+"' and abpcs ='"+bpcs_gen.elementAt(ii).toString()+"' order by line_no");
						ResultSet rsOrderEntry=stmt.executeQuery("select * from cs_ads_order_entry where quote_no='"+order_no+"' and line_no='"+line_item.elementAt(ii).toString()+"' and abpcs ='"+bpcs_gen.elementAt(ii).toString()+"' order by line_no");
						if(rsOrderEntry !=null){
							while(rsOrderEntry.next()){
//out.println(rsOrderEntry.getString("qty")+"::<BR>");
								ads_qty=for13.format(new Double(rsOrderEntry.getString("qty")).doubleValue());
								ads_qty_up=new Double(rsOrderEntry.getString("qty")).doubleValue();
								ads_price=new Double(rsOrderEntry.getString("total_sell")).doubleValue();
								ads_bpcs_item=rsOrderEntry.getString("abpcs");
							}
						}
/*
						//out.println(line_item.elementAt(ii).toString()+"::"+bpcs_part_no.elementAt(ii).toString()+"::<BR>");
						int partcounter=1;
						ResultSet rspartcounter=stmt.executeQuery("select count(*) from csquotes where order_no='"+order_no+"' and line_no='"+line_item.elementAt(ii).toString()+"' and bpcs_part_no='"+bpcs_part_no.elementAt(ii).toString()+"' and block_id='a_core'");
						if(rspartcounter != null){
							while(rspartcounter.next()){
								partcounter=rspartcounter.getInt(1);
							}
						}
						rspartcounter.close();
						if(partcounter==0){
						...
						ResultSet rsADS=stmt.executeQuery("select * from csquotes where order_no='"+order_no+"' and line_no='"+line_item.elementAt(ii).toString()+"' and bpcs_part_no='"+bpcs_part_no.elementAt(ii).toString()+"'");
						if(rsADS != null){
							while(rsADS.next()){
								ads_qty=for13.format(new Double(rsADS.getString("qty")).doubleValue()/partcounter);
								ads_qty_up=new Double(rsADS.getString("qty")).doubleValue()/partcounter;
								ads_price=new Double(rsADS.getString("extended_price")).doubleValue()/partcounter;
								ads_bpcs_item=rsADS.getString("bpcs_gen");
							}
						}
						rsADS.close();

*/
						blockId=block_id.elementAt(ii).toString();
						ads_com_price=ads_com_price+Double.parseDouble(fact_per.elementAt(ii).toString())*ads_price;
						bpcs_tier_line=bpcs_tier_lines.elementAt(ii).toString();
						//out.println(ads_price+"::<BR><BR><BR>");
					}
					if (ads_price>0){
						//out.println(ii+"::HEREx<BR>");
						//out.println(blockId+"< inside efs_price_mat_grid<BR>");
						r="";
						for (int i = 0; i < ads_qty.length(); i ++) {
							if (ads_qty.charAt(i) != '.' && ads_qty.charAt(i) != ',') r += ads_qty.charAt(i);
						}
						ads_qty=r;
						if(ads_qty.length()<11){
							String tv="";
							for(int v=0;v<(11-ads_qty.length());v++){
								tv="0"+tv;
							}
							ads_qty=tv+ads_qty;
						}
						//out.println(ads_qty);
						if(ads_bpcs_item.length()<15){
							String tv="";
							for(int v=0;v<(15-ads_bpcs_item.length());v++){
								tv=" "+tv;
							}
						ads_bpcs_item=ads_bpcs_item+tv;
						}
						double ads_com_perc=(ads_com_price/ads_price)*100;
						lineOver=ads_price;
						String ads_price_string= String.valueOf(for14.format(ads_price));
						String ads_com_price_string= for12.format(ads_com_price);
						String ads_com_perc_string= for19.format(ads_com_perc);
						String ads_price_string_up = String.valueOf(for14.format(ads_price/ads_qty_up));
						//out.println("price::"+ads_price+"QTY:"+ads_qty_up+"Unit price::"+ads_price_string_up+"<br>");
						//String comPerc=String.valueOf(for19.format(100*(ads_com_price/ads_price)));
						String comPerc="";
						//if (prio.equals("E")){
							//comPerc=String.valueOf(for19.format(100*(ads_com_price/ads_price)));
						//}
						//else{
							comPerc=String.valueOf(for19.format(100*(ads_com_price/(ads_price*(0.91)))));
						//}
						r="";
						...
						r="";
						for (int i = 0; i < ads_price_string.length(); i ++) {
							if (ads_price_string.charAt(i) != '.' && ads_price_string.charAt(i) != ',') r += ads_price_string.charAt(i);
						}
						ads_price_string=r;
						if(ads_price_string.length()<14){
							String tv="";
							for(int v=0;v<(14-ads_price_string.length());v++){
								tv="0"+tv;
							}
							ads_price_string=tv+ads_price_string;
						}
						r="";
						for (int i = 0; i < ads_price_string_up.length(); i ++) {
							if (ads_price_string_up.charAt(i) != '.' && ads_price_string_up.charAt(i) != ',') r += ads_price_string_up.charAt(i);
						}
						ads_price_string_up=r;
						if(ads_price_string_up.length()<14){
							String tv="";
							for(int v=0;v<(14-ads_price_string_up.length());v++){
								tv="0"+tv;
							}
							ads_price_string_up=tv+ads_price_string_up;
						}
						r="";
						for (int i = 0; i < ads_com_price_string.length(); i ++) {
							if (ads_com_price_string.charAt(i) != '.' && ads_com_price_string.charAt(i) != ',') r += ads_com_price_string.charAt(i);
						}
						ads_com_price_string=r;
						if(ads_com_price_string.length()<15){
							String tv="";
							for(int v=0;v<(15-ads_com_price_string.length());v++){
								tv="0"+tv;
							}
							ads_com_price_string=tv+ads_com_price_string;
						}
						r="";
						for (int i = 0; i < ads_com_perc_string.length(); i ++) {
							if (ads_com_perc_string.charAt(i) != '.' && ads_com_perc_string.charAt(i) != ',') r += ads_com_perc_string.charAt(i);
						}
						ads_com_perc_string=r;
						if(ads_com_perc_string.length()<12){
							String tv="";
							for(int v=0;v<(12-ads_com_perc_string.length());v++){
								tv="0"+tv;
							}
							ads_com_perc_string=tv+ads_com_perc_string;
						}
						double over_price=lineOver*(Double.parseDouble(overage)/(config_price-nonCommission));
						if(new Double(ads_com_price_string).doubleValue() <=0){
							over_price=0;
						}
						...
						line_no_text=String.valueOf((diLineCounter+1));
					...
					add_release_bpcs=add_release_flag.elementAt(ii).toString();
					//out.println("<BR>::"+add_release_bpcs+"::<BR>");
					final_oi_out=final_oi_out+"IO"+order_no+ads_bpcs_item+ads_qty+ads_price_string_up+ware_house_line+bpcs_tier_line+add_release_bpcs+ship_date+"\r\n";
					final_ic_out=final_ic_out+"DI"+order_no+line_no_text+ads_bpcs_item+ads_price_string_up+rep_no+"001"+comPerc+ads_com_price_string+"100000000000"+over_price_string+fc_value+"\r\n";
				...
				ads_price=0;ads_bpcs_item="";ads_qty="";ads_com_price=0;
			}//outer for
		}// product is ADS
		...
		if(Double.parseDouble(setup_cost)>0){
			//String setup_part="MINOR"+product_id;
                        String setup_part="ADSCOORDINATION";
			...
			final_oi_out=final_oi_out+"IO"+order_no+setup_part+setup_bpcs_qty+setup_cost+ware_house_line+bpcs_tier_order+add_release_bpcs+ship_date+"\r\n";
		}//setup cost
		// Getting the freight_cost
		if(Double.parseDouble(freight_cost)>0){
			String freight_part="FREIGHT"+"ADS";
		...
		else if(Double.parseDouble(handling_cost)>0){
			String freight_part="FREIGHT"+"ADS";
			String freight_bpcs_qty="1.000";
			r="";
			for (int i = 0; i < freight_bpcs_qty.length(); i ++) {
			...
			final_oi_out=final_oi_out+"IO"+order_no+freight_part+freight_bpcs_qty+handling_cost+ware_house_line+bpcs_tier_order+add_release_bpcs+ship_date+"\r\n";
		}//handling cost
		//**************
		// end oi
		//**************
		...
		if(order_notes != null && order_notes.trim().length()>0 ){
			...
			if(cust_oo.length()>50){				
				while(cust_oo.length()>50){					
					...
					r="";
					for (int j = 0; j < isti.length(); j ++) {
						if (isti.charAt(j) != '.' && isti.charAt(j) != ',') 
							r += isti.charAt(j);
					}
					isti=r;
					if(isti.length()<4){
			...
			/**
			For: OWS-870			
			Iterate records and write note for each
			D_NOTES, DI_NOTES, DA_NOTES
		*/		
		String noteLineNoVal = "";
		if(dNotes!=null&&!dNotes.isEmpty())
		{
			notesCounter=1;
			for (DNOTE dNote : dNotes) {

				if (null != dNote.getBlockID() && !dNote.getBlockID().isEmpty() && null != dNote.getDescription()
						&& !dNote.getDescription().isEmpty()) {
					String noteLineNo = dNote.getNotesLineNumber();
					String cust_oo = dNote.getBlockID() + ": " + dNote.getDescription();
					r = "";
					//Remove Carraige return and new lines from notes text 
					for (int j = 0; j < cust_oo.length(); j++) {
						if (cust_oo.charAt(j) != '\r' && cust_oo.charAt(j) != '\n') {
							r += cust_oo.charAt(j);
						}
					...
					if (cust_oo.length() > 50) {						
						while (cust_oo.length() > 50) {
							...
							if(noteLineNo.length()==1){
								//out.println("1st loop: "+noteLineNo);
								noteLineNo="00"+noteLineNo;
							}else if(noteLineNo.length()==2){
								//out.println("2nd loop: "+noteLineNo);
								noteLineNo="0"+noteLineNo;
							}else{
								noteLineNo=noteLineNo;
							}
							
							if(noteLineNo.equals(noteLineNoVal)){								
							}else if(!(notesCounter==1)){
								notesCounter=1;
								isti="0001";								
							}
							noteLineNoVal = noteLineNo;
							
							final_on_out = final_on_out + "NO" + order_no + noteLineNo + isti + cust_ooo + "\r\n";
							notesCounter++;
						}
					}
					if (cust_oo.length() <= 50 && cust_oo.length() > 0) {
						...
						noteLineNoVal = noteLineNo;
						final_on_out = final_on_out + "NO" + order_no + noteLineNo + isti + cust_oo + "\r\n";
						notesCounter++;
					} // order seq no
						// out.println("no6<BR>");
						// out.println("Test"+bpcs_cust_no);
		...
		for(int y=0; y<bpcsNote.size(); y++){
			String line_no=bpcsNoteLineNo.elementAt(y).toString();
			String tv1="";
			for(int v=0; v<(3-line_no.length()); v++){ tv1="0"+tv1;}
			line_no=tv1+line_no;

			int notesCounter2=1;
			String cust_oo=bpcsNote.elementAt(y).toString();

			if(cust_oo.length()>50){
				while(cust_oo.length()>50){
					isti=String.valueOf(notesCounter2);
					...
					final_on_out=final_on_out+"NO"+order_no+line_no+isti+cust_ooo+"\r\n";
					notesCounter2++;
				}
			}
			if(cust_oo.length()<=50 && cust_oo.length() >0){
				...
				cust_oo=cust_oo+tv1;
				final_on_out=final_on_out+"NO"+order_no+line_no+isti+cust_oo+"\r\n";
				notesCounter2++;
			}
		...
		for(int ii=0;ii<line_item.size();ii++){
			for(int i=0;i<block_id.size();i++){
				if(line_item.elementAt(i).toString().equals(line_item.elementAt(ii).toString())){
...
//out.println("before"+com_perc+"::<BR>");
//out.println(totcomm_dol+"::"+totprice_dis+":::"+nonCommission+"::<BR>");
//if (prio.equals("E")|prio.equals("D")){//deco
...
/*
		...
		ResultSet rsAdsPriceCalc=stmt.executeQuery("select freight,commission,commissionDollars from cs_ads_price_calc where order_no='"+order_no+"' and model='GLOBAL'");
		if(rsAdsPriceCalc != null){
			while(rsAdsPriceCalc.next()){
				fc_value=for12.format(new Double(rsAdsPriceCalc.getString("freight")).doubleValue());
				totcomm_dol=new Double(for12.format(new Double(rsAdsPriceCalc.getString("commissionDollars")).doubleValue())).doubleValue();
				com_perc=for19.format(new Double(rsAdsPriceCalc.getString("commission")).doubleValue());
			}
		}
		rsAdsPriceCalc.close();
...
//out.println(com_perc+"::<BR>");
		//out.println("<br> this line isn't going to work because this is based on one part per line..  that doesn'twork for ads.  1<BR>");
...
/*
		...
		if((bpcs_order_no.trim().equals("")||bpcs_order_no==null)&&((quote_origin==null)||!(quote_origin.startsWith("change")|quote_origin.startsWith("release")|quote_origin.startsWith("revision")|quote_origin.startsWith("submittal")))){
			...
			//out.println(order_no+"::"+base_quote_no);
		...
		////out.println(order_no+"::1<BR>");
		//out.println(base_quote_no+":: 1base quote number<BR>");
		if(base_quote_no==null|| base_quote_no.trim().length()==0){
			base_quote_no=order_no;
		}
		String tempend2=base_quote_no.substring(6,9);
		//out.println(base_quote_no+":: 2base quote number<BR>");
		//out.println(order_no+"::2<BR>");
...
*/
		...
		Vector erapid_quote_line=new Vector();Vector erapid_quote_linex=new Vector();Vector bpcs_parent=new Vector();Vector bpcs_child=new Vector();Vector bpcs_child_qty=new Vector();
		...
		dir_path="D:\\TRANSFER\\BPCS_BOM\\";String final_bom_out="";String final_rou_out="";dir_path1="D:\\TRANSFER\\BPCS_BOM\\test\\ADS\\";
		out.println("BOM's transfer start......<br>");
		int tempLineNo=0;
		String temperapidlineno="";
		String temppair_count="";
		sql_one="SELECT * FROM cs_bpcs_mbm where doc_number like '"+order_no+"' and doc_line in ("+lines_final+") order by cast(doc_line as integer),pair_count,bprod";
		ResultSet rs_mbm = stmt.executeQuery(sql_one);
		if (rs_mbm !=null) {
			while (rs_mbm.next()){
				if(!temperapidlineno.equals(rs_mbm.getString ("doc_line"))||!temppair_count.equals(rs_mbm.getString ("pair_count"))){
					tempLineNo++;
				}
				temperapidlineno=rs_mbm.getString ("doc_line");
				temppair_count=rs_mbm.getString ("pair_count");
				erapid_quote_linex.addElement(""+tempLineNo);
		...
		rs_mbm.close();
		//String temp_bpcs_parent="";
		//String temp_erapid_quote_line="";
		//int tempLineNo=0;
		for(int im=0;im<bpcs_parent.size();im++){
			/*
			if(im==0 || !(temp_bpcs_parent.equals(bpcs_parent.elementAt(im).toString())&&temp_erapid_quote_line.equals(erapid_quote_line.elementAt(im).toString())) ){
				temp_bpcs_parent=bpcs_parent.elementAt(im).toString();
				temp_erapid_quote_line=erapid_quote_line.elementAt(im).toString();
				tempLineNo++;
			}
			*/
			//final count
			//out.println(temp_bpcs_parent+"::<BR>");
			//out.println(temp_erapid_quote_line+"::<BR>");
			//out.println(tempLineNo+"::<BR><BR>");
			...
			//line_no
			//String erapid_line_no=""+tempLineNo;
			String erapid_line_no=""+erapid_quote_linex.elementAt(im).toString();
			//erapid_quote_line.elementAt(im).toString();
					 ...
					 if(bpcs_order_no.trim().equals("")&&!(quote_origin.startsWith("change")|quote_origin.startsWith("release")|quote_origin.startsWith("revision")|quote_origin.startsWith("submittal"))){
						 final_bom_out=final_bom_out.toUpperCase();
							...
							while(lastIndex != -1){
								 ...
								 if( lastIndex != -1){
									   countv ++;
									   String tempend3=base_quote_no.substring(6,9);
									    // final_bom_out=final_bom_out.substring(0,lastIndex)+"_00"+final_bom_out.substring(lastIndex+3,final_bom_out.length());
									   final_bom_out=final_bom_out.substring(0,lastIndex)+tempend3+final_bom_out.substring(lastIndex+3,final_bom_out.length());
								 }
							}
						}

					//BOM out put to text files
					String tempend4=base_quote_no.substring(6,9);
					// BufferedWriter out_bom = new BufferedWriter(new FileWriter(dir_path+"\\"+"B"+order_no.substring(0,6)+"_00"+".txt"));
					BufferedWriter out_bom = new BufferedWriter(new FileWriter(dir_path+"\\"+"B"+order_no.substring(0,6)+tempend4+".txt"));
		...
		dir_path="D:\\TRANSFER\\BPCS_ROU\\";dir_path1="D:\\TRANSFER\\BPCS_ROU\\test\\ADS\\";
		...
		Vector erapid_rou_quote_linex=new Vector();
		int tempLineNoRou=0;
		String temperapidlinenoRou="";
		String temppair_countRou="";
		//out.println("HEREx2");
		sql_one="SELECT * FROM cs_bpcs_frt where doc_number like '"+order_no+"' and doc_line in ("+lines_final+") order by cast(doc_line as integer),pair_count,rprod";
		ResultSet rs_frt = stmt.executeQuery(sql_one);
		if (rs_frt !=null) {
			while (rs_frt.next()){
				if(!temperapidlinenoRou.equals(rs_frt.getString ("doc_line"))||!temppair_countRou.equals(rs_frt.getString ("pair_count"))){
					tempLineNoRou++;
				}
				temperapidlinenoRou=rs_frt.getString ("doc_line");
				temppair_countRou=rs_frt.getString ("pair_count");
				erapid_rou_quote_linex.addElement(""+tempLineNoRou);
				//out.println("HERE");
		...
		//out.println("HEREx3");;
		/*
		String temp_bpcs_rou_parent="";
		String temp_erapid_rou_quote_line="";
		int tempLineNox=0;
		*/
		int roCount=1;
		String oldLineNo="";
		for(int im=0;im<bpcs_rou_parent.size();im++){
			/*
			if(im==0 || !(temp_bpcs_rou_parent.equals(bpcs_rou_parent.elementAt(im).toString())&&temp_erapid_rou_quote_line.equals(erapid_rou_quote_line.elementAt(im).toString())) ){
				temp_bpcs_rou_parent=bpcs_rou_parent.elementAt(im).toString();
				temp_erapid_rou_quote_line=erapid_rou_quote_line.elementAt(im).toString();
				tempLineNox++;
			}
			*/
			//out.println(temp_bpcs_rou_parent+"::<BR>");
			//out.println(temp_erapid_rou_quote_line+"::<BR>");
			//out.println(tempLineNox+"::<BR><BR>");
			//final count
			if(!oldLineNo.equals(erapid_rou_quote_line.elementAt(im).toString())){
				roCount=1;
			}
			String final_count=roCount+"";
			roCount++;
			oldLineNo=erapid_rou_quote_line.elementAt(im).toString();
			if(final_count.length()<3){
				String tv="";
				for(int v=0;v<(3-final_count.length());v++){
					tv=tv+"0";
			...
			//line_no
			//String erapid_line_no=""+tempLineNox;
			String erapid_line_no=""+erapid_rou_quote_linex.elementAt(im).toString();
		//	out.println(erapid_line_no);
			 ...
			 if(bpcs_order_no.trim().equals("")&&!(quote_origin.startsWith("change")|quote_origin.startsWith("release")|quote_origin.startsWith("revision")|quote_origin.startsWith("submittal"))){
				 final_rou_out=final_rou_out.toUpperCase();
					...
					while(lastIndex != -1){
						 ...
						 if( lastIndex != -1){
							   countv ++;
							   String tempend5=base_quote_no.substring(6,9);
							   //final_rou_out=final_rou_out.substring(0,lastIndex)+"_00"+final_rou_out.substring(lastIndex+3,final_rou_out.length());
							   final_rou_out=final_rou_out.substring(0,lastIndex)+tempend5+final_rou_out.substring(lastIndex+3,final_rou_out.length());
						 }
					}
				}
			//ROU out put to text files
			String tempend6=base_quote_no.substring(6,9);
			 //BufferedWriter out_rou = new BufferedWriter(new FileWriter(dir_path+"\\"+"R"+order_no.substring(0,6)+"_00"+".txt"));
			 BufferedWriter out_rou = new BufferedWriter(new FileWriter(dir_path+"\\"+"R"+order_no.substring(0,6)+tempend6+".txt"));
...
String name=request.getParameter("userSessionId");
		...
		<form name='selectform' method='post'>
			...
			<input type="hidden" name="section_transfer" value="<%=section_transfer%>">
		...
		<//jsp:include page="sales_confirm.jsp">
		<//jsp:param name="order_no" value="<%= order_no %>" />
		<//jsp:param name="rep_no" value="<%= thisRep%>" />
		<///jsp:include>
