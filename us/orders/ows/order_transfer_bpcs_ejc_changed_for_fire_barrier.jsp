		...
		//variables for bank lines end
						if(product_id.equals("EFS")){
							...
							//change done for the bank line
											Vector ejc_bpcs_item=new Vector();
											Vector blockId=new Vector();
											String line_no_text="";String blockId2="";String ejc_bpcs_item2="";String ejc_qty="";double ejc_qty_up=0; double ejc_price=0;double ejc_com_price=0;
											String bpcs_tier_line="";
											//Vars
												for(int ii=0;ii<items.size();ii++){
													//out.println("<BR> Outer"+block_id.elementAt(ii).toString()+"::"+bpcs_part_no.elementAt(ii).toString()+" here<BR>");
													blockId.clear();
													ejc_bpcs_item.clear();
												for(int i=1;i<block_id.size();i++){
														if(line_item.elementAt(i).toString().equals(items.elementAt(ii).toString())){
															if( !(block_id.elementAt(i).toString().startsWith("E_DIM")|block_id.elementAt(i).toString().startsWith("A_APRODUCT")|block_id.elementAt(i).toString().startsWith("D_NOTES")) ){
																if(block_id.elementAt(i).toString().startsWith("PRICING")){//MAT/GRID PARTS
																blockId.addElement(block_id.elementAt(i).toString());
																	//blockId=block_id.elementAt(i).toString();
																	//out.println("<BR> Inner"+block_id.elementAt(i).toString()+"::"+bpcs_part_no.elementAt(i).toString()+" here<BR>");
		//															if(win_loss.equals("RF")){//if
																		out.println(bpcs_part_no.elementAt(i).toString()+"< inside bpcs_part_no<BR>"+i+"<BR>");
																		ejc_bpcs_item.addElement(bpcs_part_no.elementAt(i).toString());
																		//ejc_bpcs_item=bpcs_part_no.elementAt(i).toString();
																		//out.println(ejc_bpcs_item.elementAt(i).toString()+"< inside bpcs_part_no<BR>");
		//													  		}
		//													  		else{
																		ejc_bpcs_item.addElement(bpcs_gen.elementAt(i).toString());
		//													  			//ejc_bpcs_item=bpcs_gen.elementAt(i).toString();
		//													  		}
																	ejc_qty=String.valueOf(for13.format(new Double(bpcs_qty.elementAt(i).toString()).doubleValue()));
																	ejc_qty_up=new Double(bpcs_qty.elementAt(i).toString()).doubleValue();
																	//out.println("1:"+price.elementAt(i).toString()+"<BR>");
																	ejc_price=ejc_price+Double.parseDouble(price.elementAt(i).toString());
																	ejc_com_price=ejc_com_price+Double.parseDouble(fact_per.elementAt(i).toString())*Double.parseDouble(price.elementAt(i).toString());
																	//out.println(efs_price_mat_grid+" efs price mat<BR>");
																	add_release_bpcs=add_release_flag.elementAt(i).toString();
																}
															}//inner if statment
														}//outer if statment
														bpcs_tier_line=bpcs_tier_lines.elementAt(i).toString();
													}//INNER for loop
													//out.println("Size = "+blockId.size());
															for(int j=0;j<blockId.size();j++){
																
																out.println("<BR> "+ejc_bpcs_item.elementAt(j).toString()+"::"+blockId.elementAt(j).toString()+" here<BR>");
																    ...
																    if (ejc_price>0){
																		//out.println(blockId+"< inside efs_price_mat_grid<BR>");
																			...
																			if(ejc_bpcs_item.elementAt(j).toString().length()<15){
																			String tv="";
																			for(int v=0;v<(15-ejc_bpcs_item.elementAt(j).toString().length());v++){
																			tv=" "+tv;
																			   }
																			ejc_bpcs_item2=ejc_bpcs_item.elementAt(j).toString()+tv;
																			}
																		...
																		if(new Double(ejc_qty).doubleValue()>0){
																			final_oi_out=final_oi_out+"IO"+order_no+ejc_bpcs_item2+ejc_qty+ejc_price_string_up+ware_house_line+bpcs_tier_line+add_release_bpcs+ship_date+"\r\n";;
																		...
																		ejc_price=0;ejc_bpcs_item2="";ejc_qty="";ejc_com_price=0;
