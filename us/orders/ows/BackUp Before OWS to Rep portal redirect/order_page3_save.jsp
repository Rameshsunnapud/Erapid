...
try{
	...
	PreparedStatement update_payment = myConn.prepareStatement(insert);
			...
			update_payment.setString(8,arch_phone);
...
if((project_type != null && (project_type.equals("PSA")||project_type.equals("SFDC")))||product.equals("GCP")||product.equals("ADS")){
	if(product.equals("ADS")){
%>
<input type='hidden' name='workcopylink' value='http://<%=application.getInitParameter("HOST")%>/erapid/us/quotes/show_summary2.jsp?orderNo=<%=order_no%>'&pid=1>
<%
}
else if (product.equals("GCP")){
%>
<input type='hidden' name='workcopylink' value='http://<%=application.getInitParameter("HOST")%>/erapid/us/reports/show_summary2.jsp?orderNo=<%=order_no%>'&pid=1>
<%
}
else{
%>
<input type='hidden' name='workcopylink' value='http://<%=application.getInitParameter("HOST")%>/erapid/us/reports/work_copy_home_psa.jsp?orderNo=<%=order_no%>'>
<%
}
}
else{
...
%>
<input type='hidden' name='repworkcopylink' value='http://<%=application.getInitParameter("HOST")%>/erapid/us/reports/work_copy_home.jsp?orderNo=<%=order_no%>'>

<input type='hidden' name='owslink' value='http://<%= application.getInitParameter("HOST")%>/erapid/us/orders/ows/order_sheet.jsp?sections=all&orderNo=<%= order_no %>&rep_no=<%= rep_no %>'>
<%
