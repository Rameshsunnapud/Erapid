						...
						<select name='cs_companyx' onchange='checkCompany()' disabled='disabled'>
							...
							<td ><select name='cs_company' onchange='checkCompany()'>
								...
								if(group_id.toUpperCase().startsWith("CAN")){
									...
									company[3] = "DL -  CS Eldercare (decolink)";
									...
									company[4] = "DL - CS Eldercare (decolink)";
	...
	out.println(product+"::"+georder);
		...
		String selected = (sregion[i].startsWith(sales_region)) ? "selected" : "";
				...
				<%
					...
					String[] sregion = {"EA--GCP","MD--GCP","WW--GCP"};
					for (int i = 0; i < sregion.length; i++) {
					String selected = (sregion[i].startsWith(sales_region)) ? "selected" : "";
				%>
				<option value='<%= sregion[i] %>' <%= selected %>><%= sregion[i] %></option>
...
if(!group_id.toUpperCase().startsWith("CAN")){
				...
				<tr><td  NOWRAP align='right'>OVERAGE</td>
...
if(fp.equals("rp") || project_type.equals("SFDC") || project_type.equals("RP")){
	String urlTemp="https://na1.salesforce.com/apex/TestLink?quoteNO="+order_no+"0";

	if(application.getInitParameter("HOST").toUpperCase().indexOf("DEV")>0){
		urlTemp="https://maya-cscrm.cs3.force.com/apex/TestLink?quoteNo="+order_no+"0";
	}

			%>

			<input type='button' class='button' name='CLOSE' value='Quote Detail'   onclick='closeThis("<%=urlTemp%>")'>
			<%
			}
			else{
			%>
