...
if(!group_id.toUpperCase().startsWith("REP")){
	if(resendlinks.trim().length()>0&&isGood){
		...
		if(product.equals("ADS")){
					%>
				<tr><td colspan='3'><input type='button' name='adstransfer' value='SEND ADS ORDER SHEET TO IMS' onclick="javascript:showPopup3('http://<%= application.getInitParameter("HOST")%>/erapid/us/orders/ows/ads_order_send.jsp?order_no=<%= order_no %>')"></td></tr>
						<%
					}
		...
		if(counter >0){
			...
			<table border='0' align='right'>
				<tr>
					...
					<td COLSPAN='3' align='right'>| <a href="javascript:showPopup2('http://<%= application.getInitParameter("HOST")%>/erapid/us/orders/ows/email_order_sheet.jsp?sections=all&order_no=<%= order_no %>&rep_no=<%= rep_no %>&userId=<%=usession_user_id%>&product=<%= product%>&Job_loc=<%=Job_loc%>&project_state=<%=project_state%>&line=<%= line%>&handling_cost=<%=handling_cost%>&setup_cost=<%=setup_cost%>&freight_cost=<%=freight_cost%>&overage=<%=overage%>&totmat_price=<%=totmat_price%>&commission=<%=commission%>')">Email Order Sheet</a></td>
					<td COLSPAN='3' align='right'>| <a href="javascript:showPopup2('http://<%= application.getInitParameter("HOST")%>/erapid/us/orders/ows/order_sheet.jsp?sections=all&orderNo=<%= order_no %>&rep_no=<%= rep_no %>&userId=<%=usession_user_id%>&product=<%= product%>&Job_loc=<%=Job_loc%>&project_state=<%=project_state%>&line=<%= line%>&handling_cost=<%=handling_cost%>&setup_cost=<%=setup_cost%>&freight_cost=<%=freight_cost%>&overage=<%=overage%>&totmat_price=<%=totmat_price%>&commission=<%=commission%>')">Order Sheet</a></td>
					<td COLSPAN='3' align='right'>| <a href="javascript:showPopup2('http://<%= application.getInitParameter("HOST")%>/erapid/us/orders/ows/credit_card_form.jsp?order_no=<%= order_no %>')">Credit Card Form</a></td>
			...
			%>
			<input type='hidden' name='workcopylink' value='http://<%=application.getInitParameter("HOST")%>/erapid/us/reports/work_copy_home_psa.jsp?q_no=<%=order_no%>'>
			<%
		}
		else{
			%>
			...
			%>
			<input type='hidden' name='repworkcopylink' value='http://<%=application.getInitParameter("HOST")%>/erapid/us/reports/work_copy_home.jsp?q_no=<%=order_no%>'>
			<input type='hidden' name='owslink' value='http://<%= application.getInitParameter("HOST")%>/erapid/us/orders/ows/order_sheet.jsp?sections=all&orderNo=<%= order_no %>&rep_no=<%= rep_no %>&userId=<%=usession_user_id%>&product=<%= product%>&Job_loc=<%=Job_loc%>&project_state=<%=project_state%>&line=<%= line%>&handling_cost=<%=handling_cost%>&setup_cost=<%=setup_cost%>&freight_cost=<%=freight_cost%>&overage=<%=overage%>&totmat_price=<%=totmat_price%>&commission=<%=commission%>'>
			<input type='hidden' name='owsurl' value='http://<%= application.getInitParameter("HOST")%>/erapid/us/orders/ows/order_sheet.jsp?orderNo=<%= order_no %>&rep_no=<%= rep_no %>&userId=<%=usession_user_id%>&product=<%= product%>&Job_loc=<%=Job_loc%>&project_state=<%=project_state%>&line=<%= line%>&handling_cost=<%=handling_cost%>&setup_cost=<%=setup_cost%>&freight_cost=<%=freight_cost%>&overage=<%=overage%>&totmat_price=<%=totmat_price%>&commission=<%=commission%>&sections='>
...
if(fpx.equals("rp")){
			...
			<input type='button' class='button' name='CLOSE' value='Rep Portal page'   onclick='closeThis()'>
			<%
			}
			else{
			%>
	...
	out.println("<a href=\"javascript:n_window('http://"+ application.getInitParameter("HOST")+"/erapid/us/legacy/sections.home.jsp?q_no="+ order_no+"&cmd=1&close=true')\">Quote Sections"+"<a>");
	out.println("</td>");
	out.println("</tr>");
	out.println("</table>");
	}
if(project_type.equals("SFDC")){
	...
	if(application.getInitParameter("HOST").toUpperCase().indexOf("DEV")>0){
		out.println("<a href='https://c.cs3.visual.force.com/apex/TestLink?quoteNO="+order_no+"1' target='_top' ><img border='0' src='../../../images/sfdc_image1.jpg'></a>");
	}
	else{
		out.println("<a href='https://na1.salesforce.com/apex/TestLink?quoteNO="+order_no+"1' target='_top' ><img border='0' src='../../../images/sfdc_image1.jpg'></a>");
	}
	}
	//else if(project_type.equals("RP")){
	//	out.println("<a href='https://maya-cscrm.cs3.force.com/apex/TestLink?quoteNO="+order_no+"1' target='_top' ><img border='0' src='../../../images/sfdc_image1.jpg'></a>");

	//}
	%>
	<%
	if(product.equals("ADS")){
String urlAds="http://"+ application.getInitParameter("HOST")+"/erapid/us/reports/work_copy_home_ads.jsp?q_no="+order_no+"&pid=3&tp=1&close=yes";
//out.println(urlAds);
