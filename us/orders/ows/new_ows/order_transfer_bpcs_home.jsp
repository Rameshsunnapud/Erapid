...
<html>
	...
	<%
			...
			String orderSheetURL = request.getParameter("orderSheetURL2");
...
boolean isGood=true;
	...
	%>
	<%@ page language="java" import="java.net.*" import="java.sql.*" import="java.io.*"  %>
   ...
   if(bpcs_order_no==null){
	...
	if(bpcs_order_no!=null && bpcs_order_no.trim().length()>0){
		...
		out.println("Validation is removed for the time being by GSO Java Team");
	}
...
System.out.println("Product Value : "+product);
	...
	<jsp:forward page="order_transfer_bpcs.jsp">
		...
		<jsp:param name="order_sheet_url" value="<%= orderSheetURL %>" />
		<jsp:param name="environment" value="<%= environment %>" />
		<jsp:param name="bpcsTransferSheet" value="<%= bpcsTransferSheet %>" />
		<jsp:param name="userId" value="<%= userIdValue %>" />
	</jsp:forward>
	<%
	}
