...
<br>
	...
	<h1>CS Order Transfer System::</h1>
		...
		//vars for io to file
					...
					String dir_path1="\\\\lebhq-erusdev\\TRANSFER\\BPCS_OE\\testing\\GCP\\";
		//
		...
		//					config_price= rs_project.getString("configured_price");
					...
					rs_project.close();
					freight_cost=""+(new Double(freight_cost).doubleValue()+new Double(handling_cost).doubleValue()); // Added by Greg and Praveen for Miranda handling issue.
...
//out.println("284 /n");
		total_sale_price=config_price+Double.parseDouble(overage)+Double.parseDouble(freight_cost)+Double.parseDouble(setup_cost);//Removed handling_cost because we added it in line 186 by Praveen Sannadi as per Miranda request
...
// 		}
		...
		//os out
									if(null!= ship_name  && ship_name.length()<30){
											String tv="";
									...
									if(null!= ship_addr1 && ship_addr1.length()<30){
											String tv="";
									...
									if(null!= ship_addr2 && ship_addr2.length()<30){
											String tv="";
											for(int v=0;v<(30-ship_addr2.length());v++){
											tv=" "+tv;
										    }
											ship_addr2=ship_addr2+tv;
									}else{ship_addr2= null!= ship_addr2? ship_addr2.substring(0,30): "";}
									if(null!=ship_city && ship_city.length()<30){
											String tv="";
											for(int v=0;v<(30-ship_city.length());v++){
											tv=" "+tv;
										    }
											ship_city=ship_city+tv;
									}else{ship_city= null!=ship_city?ship_city.substring(0,30): "";}
									if(null!= ship_state && ship_state.length()<3){
											String tv="";
											for(int v=0;v<(3-ship_state.length());v++){
											tv=" "+tv;
										    }
											ship_state=ship_state+tv;
									}else{ship_state= null!= ship_state?ship_state.substring(0,3): "";}
									if(null!=ship_zip && ship_zip.length()<9){
											String tv="";
