...
<html>
	...
	<SCRIPT language="JavaScript">
		function poponload(){
			...
			// Timer();
			var url="";
			//url="mail.delivery2.jsp?order_no="+document.selectform.order_no.value+"&to="+document.selectform.to.value+"&from="+document.selectform.from.value+"&message="+document.selectform.message.value+"&cc=&sections=all&rep_//no="+document.selectform.rep_no.value+"&name="+document.selectform.userId.value;
		...
		function poponload2()
		{
			...
			var url="transfer.jsp?sp-q="+document.selectform.order_no.value+"&cmd=5";
...
&nbsp;&nbsp;<input type='button' id='order_sheet' name='order_sheet' value='Order Write Up' onclick='openOrdersheet("<%=order_sheet_url %>")'/></center>
<br>
	...
	<h1>CS Order Transfer System::</h1>
		...
		//vars for io to file
						    ...
						    String dir_path1="\\\\lebhq-erusdev\\TRANSFER\\BPCS_OE\\testing\\EFS\\";
		//
		...
		String rep_no="";String quote_type="";String psa_quote_id="";String bpcs_tier_order="";String bpcs_order_no="";String quote_origin="";String base_quote_no="";String quote_no_origin="";
						    out.println(".......Output transfer to CS order system BPCS started for EFS....... <br><br><br>");
...
//out.println("3");
		...
		//out.println("a"+"SELECT *,REPLACE(REPLACE(CAST(Descript AS varchar(5000)), CHAR(10), ''), CHAR(13), '') AS description FROM csquotes where order_no like '"+order_no+"' and line_no in ("+lines_final+") order by cast(Line_no as integer)");
...
grid=true;
							   ...
							   if (efs_price_mat_grid>0||mat&&quote_origin.equals("sample")){
								  ...
								  //frame
								  //Template Mat Starts
								//  out.println(efs_qty_template_mat+" and "+quote_origin+"Line 1310 <br>");
...
if(Double.parseDouble(handling_cost)>0){
		...
		//			  submit_by="1234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123";
		    ...
		    if(copies_requested != null && copies_requested.trim().length()>0 && !(copies_requested.equals("0")) ){
			   ...
			   if(add_docs!=null && !add_docs.equalsIgnoreCase("") && !add_docs.equalsIgnoreCase("null") && !add_docs.isEmpty())
			   {
