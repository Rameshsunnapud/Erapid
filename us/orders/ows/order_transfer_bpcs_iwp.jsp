...
<html>
  ...
  try{
		...
		String dir_path1="D:\\TRANSFER\\BPCS_OE\\testing\\";
...
//logger.debug("c");
		...
		Vector line_whse=new Vector();
			   ...
			   while ( rs3.next() ) {
					 ...
					 desc.addElement(  (null!=rs3.getString ("Descript")?rs3.getString ("Descript"): ""  )   );
					 price.addElement(rs3.getString ("Extended_Price"));
					 //items.addElement(rs3.getString("line_no"));
...
//out.println(SELECT * FROM cs_billed_customers where order_no like '"+order_no+"::"+bpcs_cust_no+"::<BR>");
...
//logger.debug("f");
		 ...
		 */}
else if(quote_type.equals("SFDC")){
            if(source==null){
            source="";
        }
...
//logger.debug("h");
		...
		//                		if (iwp_price>0&&!(block_id.elementAt	(ii).toString().equals("A_APRODUCT"))){
								  ...
								  if (iwp_price>=0||quote_origin.toUpperCase().equals("SAMPLE")){
													 //out.println(blockId+"< inside efs_price_mat_grid<BR>");
														    ...
														    String comPerc="";
										//			out.println("Test1::"+iwp_com_price+"::"+iwp_price+"<br>");
														   /* if (prio.equals("E")){
																  if(iwp_price>0){
		...
		//								out.println("bpcs::"+line_no_text+"ERAPID::"+items.elementAt(ii).toString()+"<BR>");
														    r="";
											   ...
											   if( block_id.elementAt(i).toString().equals("E_DIM")){
												 ...
												 if(lp1>0){
													   for(int ik=0;ik<lp1;ik++){
														    ...
														    lc_count++;
													 //	out.println("Testa"+line_no_note);
													   }
...
//logger.debug("l");
		...
		String fc_value="";String fc_perc="";
			    ...
			    //          out.println(ii+"Line_price"+totprice+"comm $"+factor+"<br>");
								    factor=0;totprice=0;
								  }
		...
		//	out.println("Com_perc:"+com_perc+" comm dollars:"+for12.format( totcomm_dol)+"tot dollars: "+totprice_dis+"tot dollar actual:"+(totprice_dis-nonCommission)+"<br>");
		//overage perc
		...
		//	out.println("Total comm %:"+com_perc+" Total comm $:"+tot_com_dol_str+"<br>");
			   //ic out
//logger.debug("m");
			   //final out
		...
		<%
//out.println(final_ds_out+"::: final_ds_out");
		...
		//logger.debug("n");
				...
				//1) bills transfer start here
								    ...
								    dir_path="D:\\TRANSFER\\BPCS_BOM\\";String final_bom_out="";String final_rou_out="";dir_path1="D:\\TRANSFER\\BPCS_BOM\\test\\";
				...
				//Routings out put start
					   dir_path="D:\\TRANSFER\\BPCS_ROU\\";dir_path1="D:\\TRANSFER\\BPCS_ROU\\test\\";
		...
		//logger.debug("3");
								    //	out.println("Test"+thisRep+"::"+rep_no+"Order rep"+order_rep_no+"email:"+to);
		...
		//logger.debug("4");
								   ...
								   logger.debug("** "+name+"!!!!!!!!     $$$$$$$$$$     !!!!!!!!!!!!!!  "+userSession.getUserId());;
		%>
...
//logger.debug("q");
  ...
  int rocount=updateError.executeUpdate();
  updateError.close();
