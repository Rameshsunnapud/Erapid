...
Vector bpcsNoteLineNo=new Vector();
		...
		String sql_one="";
		ResultSet rs3 = stmt.executeQuery("SELECT * FROM csquotes where order_no like '"+order_no+"' and line_no in ("+lines_final+") and block_id like 'a_%' order by cast(Line_no as integer)");
		while ( rs3.next() ) {
			...
			if(product_id.equals("ADS")){
				...
				for(int ii=0;ii<line_item.size();ii++){
					...
					if(!block_id.elementAt(ii).toString().equals("A_APRODUCT")){
						...
						ResultSet rsADS=stmt.executeQuery("select * from cs_ads_pricing_output where order_no='"+order_no+"' and line_no='"+line_item.elementAt(ii).toString()+"' and part_no='"+bpcs_part_no.elementAt(ii).toString()+"'");
						if(rsADS != null){
							while(rsADS.next()){
								...
								ads_price=new Double(rsADS.getString("total_sell")).doubleValue()/partcounter;
								//out.println(bpcs_part_no.elementAt(ii).toString()+"::"+line_item.elementAt(ii).toString()+":::"+ads_qty+"::"+ads_qty_up+"::<BR>");
