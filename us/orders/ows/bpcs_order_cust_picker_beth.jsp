...
<%
		...
		//out.println(rep_no+"::"+entry+"::"+mode+"::"+entry_no);
...
<%
	...
	PreparedStatement query_customer =null;
		...
		if ((UserSession.getValue("usergroup").toString().startsWith("GCP"))){
				//out.println("YES1");
		query_customer = myConn.prepareStatement("select * from cs_customers where cust_name1 like ? and country_code='"+country_code_session+"' and created_rep_no in ('GC0','"+rep_no+"') and (dormant not like 'Y' or dormant is null) order by cust_name1");
		query_customer.setString(1,entry+"%");
	//	out.println("YES");
		}
		else if ((UserSession.getValue("usergroup").toString().startsWith("Grand"))){
				//out.println("YES1");
		query_customer = myConn.prepareStatement("select * from cs_customers where cust_name1 like ? and country_code='"+country_code_session+"' and created_rep_no like '"+rep_no+"' and (dormant not like 'Y' or dormant is null) order by cust_name1");
		query_customer.setString(1,entry+"%");
	//	out.println("YES");
		}
       ...
       //	query_customer.setString(2,rep_no);
		...
		query_customer.setString(1,entry_no+"%");
      // 	query_customer.setString(2,rep_no);
       //	out.println(entry_no);
		}
		//out.println("NO1");

       //query_customer.setString(2,rep_no);

		out.println(entry+"::"+rep_no+"<BR>");
		}
		...
		String cn = myResult.getString("contact_name");
		String cbno = myResult.getString("bpcs_cust_no");
		String order4 = myResult.getString("cust_no");
		order4=country_code_session+order4;
	%>
	<option value="<%= cn1 %> @<%= ca1 %>@<%= ca2 %>@<%= cct %>@<%= cst %>@<%= czp %>@<%= cp %>@<%= cf %>@<%= cn %>@<%= cbno %>@"> <%= cn1 %> ,<%= ca1 %><%= ca2 %> , <%= cct %>, <%= cst %>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	<%           }
	}
	%>
	</select><p>
