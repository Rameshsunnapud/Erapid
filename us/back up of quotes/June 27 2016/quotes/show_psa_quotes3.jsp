...
String gross_margin="";
String isInstall="";
String psa_req_date="";String psa_quote_desc="";String spec_level="";
...
String price1="";String price="";String rep_notes="";
// for formatting
...
for0.setMaximumFractionDigits(0);
 ...
 //out.println(order_no+"::<BR>");
		ResultSet rs_project = stmt.executeQuery("select *,len(exclusions)as v1,len(qualifying_notes) as v2 from cs_project where order_no='"+order_no+"'");
		if (rs_project !=null) {
 ...
 if(commission==null || commission.length()==0){
  ...
  if(gross_margin==null || gross_margin.length()==0){
	gross_margin ="0";
       }
      // out.println(commission+"::"+gross_margin);
...
String edate="";String project_id="";String psa_quote_type="";String psa_quote_notes="";String psa_type_of_quote="";
...
project_id=rs_psa_quotes.getString("project_id");
			...
			edate= rs_psa_quotes.getString("mbid_date");
			spec_level= rs_psa_quotes.getString("spec_level");
			if (rs_psa_quotes.getString("description") != null && !rs_psa_quotes.getString("description").toLowerCase().startsWith("no description")){
				psa_quote_desc = rs_psa_quotes.getString("description");
			}
			if(rs_psa_quotes.getString("request_date") != null){
				if(rs_psa_quotes.getString("request_date").trim().length()>=10){
					psa_req_date = rs_psa_quotes.getString("request_date").substring(0,10);
				}
				else{
					psa_req_date = rs_psa_quotes.getString("request_date");
				}
			}
...
//out.println("t"+tot_sum1+product);
if ((product.equals("EFS")|product.equals("EJC")|product.equals("IWP"))) {
...
grand_total=tot_sum1+extra_charges;
		...
		ResultSet rs_csquotes_sum1 = stmt.executeQuery("SELECT extended_price FROM CSQUOTES where order_no='"+order_no+"' ");
		if (rs_csquotes_sum1 !=null) {
         while (rs_csquotes_sum1.next()) {
					//s1=	rs_csquotes_sum1.getString(1);
					//s2= rs_csquotes_sum1.getString(2);
					//out.println(s1+"::"+s2+"::<BR>");
					//if(s1==null){tot_sum1=tot_sum1+(new Double(rs_csquotes_sum1.getString(2)).doubleValue());}
					//else {
					tot_sum1=tot_sum1+(new Double(rs_csquotes_sum1.getString(1)).doubleValue());
					//out.println(tot_sum1+"::"+rs_csquotes_sum1.getString(1)+"::<BR>");
	...
	double extra_charges=new Double(overage).doubleValue()+new Double(handling_cost).doubleValue()+new Double(freight_cost).doubleValue();
	//out.println(tot_sum1+"::"+extra_charges);
	grand_total=tot_sum1+extra_charges;
	if(product.equals("GCP")){
		configured_price=""+(new Double(configured_price).doubleValue()+extra_charges);
	}
	//out.println("tot"+tot_sum1);
}
else{

	if(product.equals("GCP")){
		double extra_chargesx=new Double(overage).doubleValue()+new Double(handling_cost).doubleValue()+new Double(freight_cost).doubleValue();
		//out.println(tot_sum1+"::"+extra_chargesx);
		grand_total=tot_sum1+extra_chargesx;
		//configured_price=""+(new Double(configured_price).doubleValue()+extra_chargesx);
		tot_sum1=new Double(configured_price).doubleValue();
...
price1=	new String().valueOf(grand_total);
// for1.setMinimumFractionDigits(2);
...
String odate="";String doc_priority="";
	ResultSet rs_eorders = stmt.executeQuery("SELECT doc_date,entry_date,doc_priority FROM doc_header where doc_number like '"+order_no+"'");
		...
		if(product.equals("LVR")|product.equals("LVR")|product.equals("LVR")){edate= rs_eorders.getString(2).substring(0,10);}
...
Vector sect_group=new Vector();Vector sect_group_lines=new Vector();Vector sect_notes=new Vector();//final description
	...
	if(sections>1){
		...
		for(int j=0;j<sections;j++){
			...
			for(int kl=0;kl<count_line;kl++){
				...
				//out.println("<BR>::"+desc_value+"::"+sect_name.elementAt(j).toString()+":: compare<BR>");
			...
			sect_group.addElement(sect_value.elementAt(j).toString());
			sect_group_lines.addElement(group_line);
			//out.println(group_line+"::<BR>");
			group_line="";
	...
	%>
	<%//@ include file="ads_template_body_psa.jsp"%>
	<%
	}
	else if(product.endsWith("GCP")){
		border_color="tableline_ejc";
	%>
