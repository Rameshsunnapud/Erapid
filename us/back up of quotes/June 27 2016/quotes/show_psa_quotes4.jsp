...
String psa_req_date="";String psa_quote_desc="";
...
String free_text="";String free_text_cs_project="";String exclusions="";String exclusions_free_text="";String qualifying_notes="";String qualifying_notes_free_text="";
...
for2.setMaximumFractionDigits(0);
 ...
 for1.setMinimumFractionDigits(2);
		...
		free_text_cs_project= rs_project.getString("free_text");
...
if(exclusions.trim().length()>0){
	if(exclusions.trim().substring(exclusions.trim().length()-1).equals(",")){
		//out.println(exclusions+"::1<BR>");
		exclusions=exclusions.trim().substring(0,exclusions.trim().length()-1);
		//out.println(exclusions+"::2<BR>");
	}
}
if(qualifying_notes.trim().length()>0){
	if(qualifying_notes.trim().substring(qualifying_notes.trim().length()-1).equals(",")){
		//out.println(qualifying_notes+"::1<BR>");
		qualifying_notes=qualifying_notes.trim().substring(0,qualifying_notes.trim().length()-1);
		//out.println(qualifying_notes+"::2<BR>");
	}
}
...
//out.println("HERE"+rep_no);
boolean isCanadian=false;
ResultSet rsUser=stmt.executeQuery("select group_id from cs_reps where rep_no='"+rep_no+"'");
if(rsUser != null){
	while(rsUser.next()){
...
if(free_text_cs_project != null){
for(int rr=0; rr<free_text_cs_project.length(); rr++){
	if((int)free_text_cs_project.charAt(rr)==10){
		free_text_cs_project=free_text_cs_project.substring(0,rr)+"<BR>"+free_text_cs_project.substring(rr);
		rr=rr+4;
	}
}
}
...
if(overage==null || overage.trim().length()<=0){overage="0";}
if(handling_cost==null||handling_cost.trim().length()<=0){handling_cost="0";}
if(freight_cost==null||freight_cost.trim().length()<=0){freight_cost="0";}
...
extra_charges=new Double(overage).doubleValue()+new Double(handling_cost).doubleValue()+new Double(freight_cost).doubleValue();
...
//out.println(rep_no+"  REP NUMBER<BR>");
...
//			out.println("v "+desc_value1+"<br>");
	...
	if(section_page==null||section_page.equals("0")||sections<=0){//cheking to print different pages
		...
		//out.println("HERE");
	%>
	<%@ include file="quote_template_header_psaCAN.jsp"%>
...
if(product.equals("ADS")){
	...
	if((product.equals("IWP")||product.equals("EJC")||product.equals("APC"))&&(section_page==null||(section_page.equals("0")))){
	//out.println("HERE");
	%>
