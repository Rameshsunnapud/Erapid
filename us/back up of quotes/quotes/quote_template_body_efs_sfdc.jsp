...
if(sections<=0){
	...
	for(int i=0; i<desc.size(); i++){
		if(!desc.elementAt(i).toString().equals(tempHeader)){
			out.println("<tr><td class='maindesc'><b>"+desc.elementAt(i).toString()+"</b></td></tr><tr><td>&nbsp;</td></tr>");
		}
		//}
		...
		if(rsframes != null){
			while(rsframes.next()){
				if(new Double(rsframes.getString(1)).doubleValue()<1){
					out.println("<tr><td class='maindesc'>No Frame</td></tr>");
				}
			}
		}
		rsframes.close();
		...
		if(rscsquotes != null){
			while(rscsquotes.next()){
				...
				out.println("<tr><td class='maindesc'>"+tempdesc+"</td></tr>");
			}
		}
		rscsquotes.close();
