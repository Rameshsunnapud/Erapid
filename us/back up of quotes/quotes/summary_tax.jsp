...
ResultSet rstaxproject=stmt.executeQuery("select tax_zip,project_type from cs_project where order_no='"+order_no+"'");
...
if(tax_perc != null && tax_perc.trim().length()>0 && !tax_perc.toUpperCase().equals("NULL")&& new Double(tax_perc).doubleValue()>0){
	...
	String query1="select * from csquotes where order_no='"+order_no+"' and not extended_price is null and not extended_price='' and cast(extended_price as float)>0";
	if(secLines != null &&  secLines.trim().length()>0){
		query1="select * from csquotes where order_no='"+order_no+"' and not extended_price is null and not extended_price='' and cast(extended_price as float)>0 and line_no in ("+secLines+")";
	}
	ResultSet rstax=stmt.executeQuery(query1);
	if(rstax != null){
		while(rstax.next()){
			if(product_id.equals("GCP")||product_id.equals("ADS")){
	...
	if(product_id.equals("GCP")||product_id.equals("ELM")){
		//t.println(totprice_dis+"::"+overage+"::"+handling_cost+"::"+freight_cost+"::"+setup_cost+"::<BR>");
		...
		if(project_type.equals("PSA")){
			...
			tempxxx=new Double(totprice_dis).doubleValue()+new Double(freight_cost).doubleValue();
		}
...
if (!(tax_perc==null||tax_perc.equals("0")||new Double(tax_perc).doubleValue()<=0)){
	...
	if(isquote.toLowerCase().equals("yes")){
		...
		if(product_id.equals("ADS")){
			...
			out.println("<td class='maindesc' width='50%' align='right' valign='middle' nowrap><b><font class='totprice'>"+df0.format(tax_dollar)+"</font></b></td>");
			out.println("</tr>");
			out.println("</table>");
			out.println("<table cellspacing='0' cellpadding='0' border='0' width='100%' height='25'><tr>");
			out.println("<td class='maindesc' width='50%' valign='middle'><b>Material and Tax Only </b></td>");
			out.println("<td class='maindesc' width='50%' align='right' valign='middle' nowrap><b>Total: <font class='totprice'>"+df0.format(grand_total)+"</font></b></td>");
			out.println("</tr>");
			...
			out.println("<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b><font class='totprice'>"+for1.format(tax_dollar)+"</font></b></td>");
			out.println("</tr>");
			out.println("</table>");
			out.println("<table class='tableline_iwp' cellspacing='0' cellpadding='0' border='0' width='100%' height='25'><tr>");
			if(product_id.equals("IWP")){
		...
		out.println("<table class='tableline' cellspacing='0' cellpadding='0' border='0' width='100%' height='25'><tr>");
			out.println("<td class='tableline_row mainbody' width='50%' valign='middle'><b>Tax Only:(Tax %: "+tax_perc+")</b></td>");
			out.println("<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b><font class='totprice'>$"+df0.format(tax_dollar)+"</font></b></td>");
			out.println("</tr>");
			out.println("</table>");
			out.println("<table class='tableline' cellspacing='0' cellpadding='0' border='0' width='100%' height='25'><tr>");
			if(product_id.equals("IWP")){
			...
			out.println("<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b>Total: <font class='totprice'>$"+df0.format(grand_total)+"</font></b></td>");
			out.println("</tr>");
