<%
	int numLeafs=0;
	ResultSet rssum=stmt.executeQuery("SELECT sum(cast (field19 as integer)) as a1 FROM csquotes WHERE order_no='"+ order_no +"' and not field18 in('EDGE','KP','LFK','AST')");
	if(rssum != null){
		while(rssum.next()){
	...
	rssum.close();
	out.println("<table border='0' width='100%' align='center'><tr><td><font class='maindesc'>Scope: C/S proposes to furnish a total of ("+numLeafs+") 1-3/4\" thick Acrovyn Door leaf(s) as listed herein.</font></td></tr></table><BR>");
	out.println("<table border='1' width='100%' align='center'><tr><td wdith='15%' class='maindesc'><b>Model</b></td><td width='10%' class='maindesc'><b>Leafs</b></td><td width='75%' class='maindesc'><b>Description</b></td></tr>");
	Vector v1=new Vector();
	Vector v2=new Vector();
	ResultSet rsa=stmt.executeQuery("select field18 as a2, sum(cast(field19 as integer)) as a1 from csquotes where order_no = '"+ order_no +"' and block_id = 'A_CORE' group by field18 order by field18");
	if(rsa != null){
		while(rsa.next()){
	...
	for(int i=0; i<v1.size();i++){
		...
		out.println("<tr><td class='maindesc'>"+v1.elementAt(i).toString()+"</td><td class='maindesc' align='right'>"+v2.elementAt(i).toString()+"</td><td class='maindesc'>"+tempDesc+"</td></tr>");
	}
	out.println("</table>");
 ...
 if(sfdcNotes != null && sfdcNotes.trim().length()>0){
	out.println("<table width='100%'><tr><td class='mainbody'><b><i>"+sfdcNotes+"</b></i></td></tr></table><BR>");
...
<table class='<%= border_color %>' cellspacing='0' cellpadding='0' border='0' width='100%' align='center' height='25'><tr>
		...
		<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b>Total: <font class='totprice'><%= for0.format(new Double(price1).doubleValue()) %></font></b></td>
	</tr>
...
<table border='0' width='100%' align='center'><tr><td>
			...
			if(qualifying_notes != null && qualifying_notes.trim().length()>0){
				ResultSet cs_qlf_notes = stmt.executeQuery("select description FROM cs_qlf_notes where product_id='ADS' and code in ("+qualifying_notes+") order by code ");
			...
			<br><b> Exclusions(s):</b><br>
			<%
			if(exclusions != null && exclusions.trim().length()>0){
				ResultSet cs_exc_notes = stmt.executeQuery("select description FROM cs_exc_notes where product_id='ADS' and code in ("+exclusions+") order by code ");
			...
			Door Coordination: If C/S is to coordinate the order add the following: $5.00 per door (new frame); $10.00 per door (existing frame).  Minimum $90.00 coordination charge applies.<br>
			...
			<br>1. C/S Standard Terms and Conditions shall apply.
			...
			<BR>7. Price includes a single delivery shipment in box trucking with standard wood crating.
			<BR>8. For projects located outside the U.S. Construction Specialties requires that all freight forwarders who handle C/S product be C-TPAT certified. Please contact your local representative for more information and a list of available C-TPAT carriers in your area.
