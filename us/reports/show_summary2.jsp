...
if(nowc==null){
	nowc="";
		...
		String session_rep_no= request.getParameter("rep_no");
		String usergroup=request.getParameter("group");
		if(usergroup == null){
			usergroup="";
		}
		if(session_rep_no==null){
			session_rep_no="";
		}
...
close="";
		if(close.equals("yes")){
			out.println("<body onload='javascript:self.blur();window.close();'>");
		}
//out.println(session_rep_no+"::"+usergroup);
%>
...
//out.println("  @411 "+"::<BR>");
        ...
        while ( rs3.next() ) {
			//out.println("  @416 "+"::<BR>"+query_string);
		if(rs3.getString("block_id").equals("A_APRODUCT")){
			if(!product.equals("GCP"))  {
				if(!rs3.getString("weight").equals(""))
				{
				tot_weight=tot_weight+Double.parseDouble(rs3.getString("weight")); //Changed by praveen for one of the issue raised by QA
				}
			}
...
//out.println(product+"::");
	if(product.equals("GCP")){
		//out.println("HERE");
%>
<%@ include file="show_summary_header_gcp.jsp"%>
<%
}
else{
//out.println("HERE");
...
//out.println("HERE In ADS");
%>
<%@ include file="ads_bpcs_output.jsp"%>
<%
}
else{
//out.println("Else HERE In ADS");
%>
<%@ include file="work_copy_ads.jsp"%>
<%
//out.println("Else HERE In ADS ENd");
}
}
