...
<html>
	...
	<body>
		...
		boolean wasSun=false;
		double totalCubicFootage=0;
		String grillecost="0";
		String vaccost="0";
		String rfcost="0";
		String econocost="0";
		...
		<p align="center"><b><%=project_name%></b></p>
				<%
					...
					for(int counter=0; counter<numberOfSections; counter++){
						...
						java.sql.ResultSet myResult=stmt.executeQuery("select * from doc_line where doc_number= '"+onumber+"' and doc_line in("+sectionLines[counter]+")"+whereClause+" order by product_id,cast(doc_line as decimal),subline_no" );



						java.sql.ResultSet myResult1=stmt1.executeQuery("select doc_line from doc_line where doc_number= '"+onumber+"' and doc_line in("+sectionLines[counter]+")"+whereClause+" group by product_id,doc_line order by cast(doc_line as decimal)" );
						java.sql.ResultSet myResult2=stmt2.executeQuery("select * from CS_COSTING where order_no= '"+onumber+"' and item_no in("+sectionLines[counter]+")"+whereClause2+" order by product_id,cast(item_no as decimal),subline_no" );
...
String tempQty=myResult2.getString("cost_flag");
			if(tempQty==null || tempQty.trim().length()==0 || tempQty.equals("null")){
				tempQty="1";
			}
if(product_id.equals("FSM")){
	qtySec=qtySec+myResult2.getDouble("qty")*new Double(tempQty).doubleValue();
}
else{
	qtySec=qtySec+myResult2.getDouble("qty")*myResult2.getDouble("stile_code")*new Double(tempQty).doubleValue();
}
...
wasSun=false;
		...
		//FINLIB_COST
			...
			if (nf >=0 ){
				...
				if(nf1<0){
				lvrfin=order2[m].substring(nf+9);
}
else{
			...
			if(product_id.equals("GRILLE")){
				if(isModular){
					...
					int ns=order2[m].indexOf("RUN1CORNERS");
					if (ns >=0 ){
						int ns1=order2[m].indexOf("]",ns);
						sec=order2[m].substring(ns+12,ns1);
						if(sec.indexOf(".")>=0){
...
shape=order2[m].substring(n_sp+6);
}
		...
		//ue())+"&nbsp;&nbsp;&nbsp;<b>SF Area:</b>"+((new Float(new Double(sfarea).doubleValue()+0.5)).intValue()
		int sysdepth=order2[m].indexOf("SYSDEPTH");
		double tempSysDepth=0;
			if (sysdepth >=0 ){
				int sysdepthi=order2[m].indexOf("]",sysdepth);
				String vt=order2[m].substring(sysdepth+9,sysdepthi);
				//out.println(vt+":<BR>");
				tempSysDepth=(new Double(vt).doubleValue()+3)/12;
				//out.println(tempSysDepth+"::"+sfarea+"::<BR>");
				//tsarea=tsarea+((new Float(vt)).floatValue());
				totalCubicFootage=totalCubicFootage+tempSysDepth*((new Float(new Double(sfarea).doubleValue()+0.5)).intValue());
				//out.println(totalCubicFootage+":::::<BR><BR>");
			}
		...
		out.println("<td align='center'>Total Sections: "+qtySec+"</td>");
		...
		out.println("<td width='3%' colspan='2' align='left' nowrap>"+"Total Cubic Footage = "+((new Float(totalCubicFootage+0.5)).intValue())+" Cubic Ft."+"</td>");
		totalCubicFootage=0;
		}
		...
		out.println("<tr><td width='6%' colspan='12' align='center'>");
		out.println("Price/Lb Target: $7.40");
		out.println("</td></tr>");
