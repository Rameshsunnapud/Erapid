<html>
	...
	<body>
		...
		if( pid.equals("1")) {
		str_qry2="SELECT description,ins_style,ins_col,cat_no,part_no,count(*) FROM CS_TEAR_SHEETS where order_no ='"+order_no+"' group by description,ins_style,ins_col,cat_no,part_no ";
						 }
		else{
		str_qry2="SELECT description,ins_style,ins_col,cat_no,part_no,count(*) FROM CS_TEAR_SHEETS where order_no ='"+order_no+"' and item_no='"+line_no+"' group by description,ins_style,ins_col,cat_no,part_no ";
		}
		...
		if (count_line>0){
			    for (int m=0;m<count_group;m++){ //OUTer for loop bEGIN
				 ...
				 if (count_no_page<=0) {
										count_no_page++;
		%>
		<%@ include file="tear.head.ejc.jsp"%>
		<%@ include file="tear.footer.ejc.jsp"%>
		<%
