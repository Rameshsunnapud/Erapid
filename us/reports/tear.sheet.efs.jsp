<html>
	...
	<body>
		...
		str_qry1="SELECT * FROM CS_TEAR_SHEETS where order_no like '"+order_no+"%' and item_no='"+line_no+"' order by cast(item_no as integer)";
				...
				if (efs_ts !=null) {
			   while (efs_ts.next()) {
				item_no.addElement(efs_ts.getString("item_no").trim());
				...
				for (int n=0;n<count_line;n++){ //inner for loop bEGIN
					if((description_group.elementAt(m).equals(description.elementAt(n)))&(ins_style_group.elementAt(m).equals(ins_style.elementAt(n)))&(ins_col_group.elementAt(m).equals(ins_col.elementAt(n)))){//If loop bEGIN
						if (count_no_page<=0) {
							...
							count_no_page++;
		%>
		<%@ include file="tear.head.efs.jsp"%>
		<%@ include file="tear.footer.efs.jsp"%>
		<%
		//out.println(" x The quantity"+new Double(qty.elementAt(n).toString()).intValue());
		//four line loop
		out.println("<tr>");
		...
		// out.println(" y The quantity"+new Double(qty.elementAt(n).toString()).intValue());
		count_no_page++;
		if (count_no_page>4){
			count_no_page=0.0;
...
// Added by AC to close html tags that remain open from tear.footer.efs.jsp - March 17, 2009
