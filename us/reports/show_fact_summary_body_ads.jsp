...
ResultSet xPrice=stmt.executeQuery("select extended_price as numeric,fcperc from csquotes where order_no='"+order_no+"' and line_no in ("+itemsLines+") and (deduct is null or deduct='add' or deduct='') ");
if(xPrice != null){
		while(xPrice.next()){
			xPriceTot=""+(new Double(xPriceTot).doubleValue()+new Double(xPrice.getString(1)).doubleValue());
                        fcDollars=fcDollars+(xPrice.getDouble(2)/100)*xPrice.getDouble(1);
                      //  out.println(fcDollars+"::<BR>");
				...
				if ((line_item.elementAt(mn).toString()).equals((items.elementAt(newIndex).toString()))){
					...
					if( !((block_id.elementAt(mn).toString()).equals("A_APRODUCT")))	{
						...
						if (std_cost.elementAt(mn)==null){
							...
							//cost_line=std_cost.elementAt(mn).toString();
							
							if(line_item.elementAt(mn).equals(items.elementAt(newIndex))){
								cost_line = lineTotalCostList.get(newIndex).toString();
							}
...
/* 							if (prio.equals("E")){
						...
						//out.println(prod_code+"::"+prodCode[codeCount]+":: product codes<BR>");
...
out.println("<td width='17%' bgcolor='#006600'colspan='8' align='center'><font class='schedule'><b>Product Summary</b></font></td></tr>");
out.println("<tr><td width='20%' valign='top' bgcolor='#006600' class='maindesc'><font class='schedule'>"+"CODE"+"</font></td>");
out.println("<td width='10%' valign='top' bgcolor='#006600' class='maindesc'  align='right'><font class='schedule'>"+"QTY"+"</font></td>");
out.println("<td width='15%' valign='top' bgcolor='#006600' class='maindesc'  align='right'><font class='schedule'>"+"COST"+"</font></td>");
out.println("<td width='15%' valign='top' bgcolor='#006600' class='maindesc'  align='right'><font class='schedule'>"+"SELL"+"</font></td>");
out.println("<td width='15%' valign='top' bgcolor='#006600' class='maindesc'  align='right'><font class='schedule'>"+"GROSS PROFIT"+"</font></td>");
out.println("<td width='15%' valign='top' bgcolor='#006600' class='maindesc'  align='right'><font class='schedule'>"+"FC $"+"</font></td>");
out.println("<td width='15%' valign='top' bgcolor='#006600' class='maindesc'  align='right'><font class='schedule'>"+"GP%"+"</font></td>");
out.println("<td width='15%' valign='top' bgcolor='#006600' class='maindesc'  align='right'><font class='schedule'>"+"COMM%"+"</font></td></tr>");
/*
...
int count=0;
ResultSet rsSum=stmt.executeQuery("select prod_code,sum (cast(extended_price as numeric)) as a1 ,sum(cast(std_cost as numeric)*cast (bpcs_qty as numeric)) as a2, "
 + "sum(((cast(extended_price as numeric)+cast(extended_price as numeric)/"+tot_sum+"*"+overage+")*(cast(wdth as numeric)/100))*"+qType+" )  as a3,SUM(CAST(bpcs_qty AS integer)) AS a4, fcperc AS a5 from csquotes where order_no = '"+order_no+"' and not block_id='d_notes' and not prod_code ='' group BY prod_code, fcperc");

if(rsSum != null){
	while(rsSum.next()){
		if ((count%2)==1){bgcolor="#e4e4e4";}else {bgcolor="#EFEFDE";}
		double sell=rsSum.getDouble("a1");
		double cost=rsSum.getDouble("a2");
		double profit=rsSum.getDouble("a1")-rsSum.getDouble("a2");
		double gm=(profit/sell)*100;
		double comm=rsSum.getDouble("a3");
		double tempQty=rsSum.getDouble("a4");
		double fcPerc = rsSum.getDouble("a5");
		
		divider=sell*qType;
		if(divider!=0){
			comm=(comm/divider)*100;
		}
		out.println("<tr><td width='20%' align='left' bgcolor='"+bgcolor+"' class='maindesc'>"+rsSum.getString("prod_code")+"  "+"</td>");
out.println("<td width='10%' align='right' bgcolor='"+bgcolor+"' class='maindesc'>"+tempQty+"</td>");
		out.println("<td width='15%' align='right' bgcolor='"+bgcolor+"' class='maindesc'>"+cost+"</td>");
		out.println("<td width='15%' align='right' bgcolor='"+bgcolor+"' class='maindesc'>"+sell+"</td>");
		out.println("<td width='15%' align='right' bgcolor='"+bgcolor+"' class='maindesc'>"+profit+"</td>");
                out.println("<td width='15%' align='right' bgcolor='"+bgcolor+"' class='maindesc'>"+ for1.format((sell*fcPerc)/100)+"</td>");
		out.println("<td width='15%' align='right' bgcolor='"+bgcolor+"' class='maindesc'>"+gm+"</td>");
		out.println("<td width='15%' align='right' bgcolor='"+bgcolor+"' class='maindesc'>"+comm+"</td></tr>");
		count++;
	}
}
rsSum.close();
...
sec_lines="";
	...
	totalpricex=totalpricex+totprice_dis+Xtra;
