...
if(group.trim().length()==0 && UserSessionx != null){
	if(UserSessionx.getAttribute("usergroup")!=null){
		group= UserSessionx.getAttribute("usergroup").toString();
	}
}
...
if(st == null || st.equals("null") || (st.equals("2")|| st.equals("3"))){
	ResultSet rsQuoteSection= stmt.executeQuery(" select * from cs_quote_sections where order_no='"+order_no+"'  and (not sections is null and not sections='1' and not sections='0')");
...
// Checking the no of lines
		...
		rs1.close();
			...
			double stdCostValue = 0;ResultSet stdCostRS = null;
			List<String> lineTotalCostList = new ArrayList<String>();
			for(int stdv=0;stdv<items.size();stdv++){
			stdCostValue = 0;
				String stdCostQuery = "SELECT line_no, cast(std_cost as decimal) as std_cost FROM csquotes WHERE order_no = '"+order_no+"' and line_no = '"+items.elementAt(stdv)+"'";				
				stdCostRS = stmt.executeQuery(stdCostQuery);
				while ( stdCostRS.next() ) {
					stdCostValue=stdCostValue+new Double(stdCostRS.getString("std_cost")).doubleValue();
				}
				lineTotalCostList.add(new Double(stdCostValue).toString());
				out.println("STD cost for line "+items.elementAt(stdv)+" is : "+stdCostValue);
			}
			stdCostRS.close();
		out.println("STD cost list : "+lineTotalCostList);			
...
%>
<%@ include file="show_fact_summary_body_ads.jsp"%>
<%
