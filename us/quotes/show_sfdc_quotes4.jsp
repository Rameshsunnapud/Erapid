...
String configured_price_for_tax="0";
...
ResultSet rsx=stmt.executeQuery("select extended_price from csquotes where Order_no like '"+order_no+"%' and product_id!='GE' ");
if(rsx != null){
	while(rsx.next()){
		if(rsx.getString(1)!= null & rsx.getString(1).trim().length()>0){
			temp_cost=temp_cost+new Double(rsx.getString(1)).doubleValue();
		}
	}
}
rsx.close();
...
exchName=rs_project.getString("exch_name");
		...
		//out.println("Configured price is "+configured_price);
	...
	if(overage==null){ overage="0";}
		...
		//price1=""+(new Double(overage).doubleValue()+new Double(handling_cost).doubleValue()+new Double(freight_cost).doubleValue());
...
*/
	...
	if (!(product.equals("LVR")|product.equals("BV")|product.equals("GRILLE")|product.equals("ADS"))) {
		...
		//out.println(grand_total+"::"+configured_price+"::"+commission+"::"+overage+"::"+setup_cost+"::"+handling_cost+"::"+freight_cost+"::<BR>");
		if(product.equals("GCP")){
			
			
			//out.println("Tax Zip : "+tax_zip);
			String state=""; boolean istaxstate=false;
...
rstaxstate.close();
			...
			if(!istaxstate)
			{
				//extra_charges=extra_charges-new Double(freight_cost).doubleValue();
				configured_price_for_tax=""+(new Double(configured_price).doubleValue()+extra_charges-new Double(freight_cost).doubleValue());
			}
...
if(product.endsWith("GCP")){
	...
	//out.println("sectioins2="+sections);
	%>
	<%@ include file="quote_template_body_gcp.jsp"%>
	<%
}
else if(product.endsWith("REF")){
	border_color="tableline_ejc";
	//out.println(sections);
	%>
	<%@ include file="quote_template_body_ref_sfdc.jsp"%>
	<%
}
