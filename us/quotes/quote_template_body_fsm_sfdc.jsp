...
if(sections<=0){
	...
	Vector model=new Vector();Vector sqft_group=new Vector();Vector model_group=new Vector();Vector screen_group=new Vector();Vector finish_group=new Vector();Vector group_count=new Vector();Vector item_all=new Vector();
	...
	Vector actuator=new Vector();Vector sqft=new Vector();
Vector vertMullions=new Vector();
Vector depth=new Vector();
	...
	if (rs_csquotes !=null) {
		while (rs_csquotes.next()) {
			...
			vertMullions.addElement(rs_csquotes.getString("recap"));
	...
	if (rs_cscosting !=null) {
		while (rs_cscosting.next()) {
			...
			sqft.addElement(rs_cscosting.getString("sqft"));
                        //out.println(rs_cscosting.getString("sqft")+"::<BR>");
			...
			if(rs_cscosting.getString("ssedge_code")!=null){
				depth.addElement(rs_cscosting.getString("ssedge_code"));
			}
			else{
				depth.addElement("");
			}
	...
	rs_cscosting.close();
	int t_group=0;
	ResultSet rs_cscosting_group = stmt.executeQuery("SELECT MODEL,FINISH,SCREEN,actuator,sskp_code,COUNT(*)as v,sum(sqft*qty) as sqft from cs_costing where order_no='"+order_no+"' and model not in ('8615F') GROUP BY MODEL,FINISH,SCREEN,actuator,sskp_code");
	if (rs_cscosting_group !=null) {
		while (rs_cscosting_group.next()) {
                    //out.println(rs_cscosting_group.getString("sqft")+":: from cscosting<BR>");
			if(t_group==0){
                            sqft_group.addElement(rs_cscosting_group.getString("sqft"));
...
//out.println(n+"::"+t_costing+"::<BR>");
				if((sskp_code_group.elementAt(mn).toString().equals(sskp_code.elementAt(n).toString()))&(model_group.elementAt(mn).toString().equals(model.elementAt(n).toString()))&(finish_group.elementAt(mn).toString().equals(finish.elementAt(n).toString()))&(screen_group.elementAt(mn).toString().equals(screen.elementAt(n).toString()))&(actuatorGroup.elementAt(mn).toString().equals(actuator.elementAt(n).toString()))){
					...
					//moved 7 lines above
						out.println("<table width='100%' cellspacing='1' cellpadding='2' border='0'>");
						for (int v=0;v<k;v++){
//out.println(line_no.elementAt(v).toString()+"::"+v+"::<BR>");
                                                                //out.println(item_all.elementAt(n).toString()+"::<BR><BR>");  
...
//out.println(numMullions.elementAt(v).toString()+":::"+v+"::<BR>");
								if(numMullions.elementAt(v)!= null){
									numMullionsTemp=numMullions.elementAt(v).toString();
								}
								if(vertMullions.elementAt(v)!= null){
									vertMullionsTemp=vertMullions.elementAt(v).toString();
								}
...
//out.println(finish.elementAt(n).toString()+"::<BR>");
						...
						//out.println("<tr width='100%'><td colspan='4' class='mainbody2'><B>MODEL:</b>"+model.elementAt(n).toString()+" </td></tr>");
						out.println("<tr width='100%'><td colspan='4' class='mainbody2'>"+finish_desc+"</td></tr>");
						out.println("<tr width='100%'>");
						//out.println("<td  class='mainbody2' width='15%'><b>Total Square Footage:</b></td>");
						//out.println("<td  class='mainbody2' width='15%'>"+for12.format(new Double(sqft_group.elementAt(mn).toString()).doubleValue())+"</td>");
						out.println("<td  class='mainbody2' width='15%'><b>PSF Wind Loading:</b></td>");
						out.println("<td  class='mainbody2' width='45%'>x</td>");
						out.println("</tr>");
						out.println("</table>");
						out.println("<table width='100%' cellspacing='1' cellpadding='2' border='0'>");
						...
						out.println("<td width='10%' bgcolor='#FFFFFF' class='schedule' align='center'><b><u>Horizontal Mullions</u></b></td>");
						out.println("<td width='10%' bgcolor='#FFFFFF' class='schedule' align='center'><b><u>Vertical Mullions</u></b></td>");
...
//out.println(n+"USING WRONG INDEX.  Do not use N:::{"+line_no.elementAt(n).toString()+"::"+mark.elementAt(n).toString()+"::<BR>");

out.println("<td valign='top' nowrap nowrap bgcolor='"+bgcolor+"' class='maindesc'  align='center'>"+numMullionsTemp+"</td>");

out.println("<td valign='top' nowrap nowrap bgcolor='"+bgcolor+"' class='maindesc'  align='center'>"+vertMullionsTemp+"</td>");

						//out.println("<td valign='top' nowrap bgcolor='"+bgcolor+"' class='maindesc' align='center'>x</td>");
						//out.println("<td valign='top' nowrap bgcolor='"+bgcolor+"' class='maindesc' align='center'>y</td>");
...
ResultSet rs_cscosting = stmt.executeQuery("select * from cs_costing where order_no='"+order_no+"' and item_no in("+sect_group_lines.elementAt(ye).toString()+") order by cast(item_no as integer)");
	if (rs_cscosting !=null) {
	   while (rs_cscosting.next()) {
		sqft.addElement(rs_csquotes.getString("sqft"));
		item_all.addElement(rs_cscosting.getString("item_no"));
		model.addElement(rs_cscosting.getString("model"));
		width.addElement(rs_cscosting.getString("width"));
		height.addElement(rs_cscosting.getString("height"));
...
<jsp:include page="psa_quote_cran_price_block.jsp" flush="true">
	...
	<jsp:param name="isquote" value="yes"/>
	<jsp:param name="product_id" value="<%= product%>"/>
	<jsp:param name="secLines" value="<%= sect_group_lines.elementAt(ye).toString()%>"/>
	<jsp:param name="sectotalname" value="<%=sectotalname %>"/>
	<jsp:param name="border_color" value="<%=border_color %>"/>
