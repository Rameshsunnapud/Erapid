...
session_rep_no= userSession.getRepNo();
session_group_id= userSession.getGroup();
if((session_rep_no==null||session_rep_no.trim().length()<=0) && UserID != null && UserID.trim().length()>0){
	ResultSet rsRep=stmt.executeQuery("select rep_no,group_id from cs_reps where user_id='"+UserID.trim()+"'");
	if(rsRep != null){
		while(rsRep.next()){
			session_rep_no=rsRep.getString(1);
			session_group_id=rsRep.getString(2);
			}
	}
	rsRep.close();
}
...
if(AcctID ==null){
	AcctID="";
	ResultSet rs_psa_quotes = stmt_psa.executeQuery("SELECT * FROM dba.cs_rep where rep_id='"+session_rep_no.trim()+"'");
	if (rs_psa_quotes !=null) {
		while (rs_psa_quotes.next()) {
			AcctID=rs_psa_quotes.getString("acct_id");
		}
	}
	rs_psa_quotes.close();
}
...
//out.println(session_rep_no+"::"+session_group_id);
