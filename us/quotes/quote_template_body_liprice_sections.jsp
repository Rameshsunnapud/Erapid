...
rsx.close();
ResultSet rsx2=stmt.executeQuery("select sum(cast(extended_price as float(2))) from csquotes where order_no='"+order_no+"' and ((Block_ID ='PRICING' and cast(Sequence_no as decimal)='0.00') or block_id='B_ADDERS') and cast(extended_price as decimal)>0 and line_no in("+sect_group_lines.elementAt(ye).toString()+") ");
//out.println("select sum(cast(extended_price as decimal)) from csquotes where order_no='"+order_no+"' and ((Block_ID ='PRICING' and cast(Sequence_no as decimal)='0.00') or block_id='B_ADDERS') and cast(extended_price as decimal)>0 and line_no in("+sect_group_lines.elementAt(ye).toString()+") ");
if(rsx2 != null){
	while(rsx2.next()){
	...
	while(rsn.next()){
		...
		adders=adders+new Double(rsn.getString("extended_price")).doubleValue();
			...
			if(isSecOverage && overageSec.elementAt(ye).toString() != null && overageSec.elementAt(ye).toString().trim().length()>0){
				...
				//out.println("<BR>Line "+line_cost+"::"+secTOTALPRICE+"::"+overageSec.elementAt(ye).toString()+"<BR>");
				//out.println("1<BR>");
			}else{
				out.println("2<BR>");
			...
			if(isSecFreight && freightSec.elementAt(ye).toString() != null && freightSec.elementAt(ye).toString().trim().length()>0){
			...
			if(isSecHandling && handlingSec.elementAt(ye).toString() != null && handlingSec.elementAt(ye).toString().trim().length()>0){
			...
			//out.println("::"+freightX+"::"+handlingX+"::"+overageX+"::<BR>");
...
//						//out.println("overage"+overage+"freight_cost"+freight_cost+"handling_cost"+handling_cost);
			...
			totalLineItemcost += (new Double(line_cost)).doubleValue();
...
<table class='<%= border_color %>' cellspacing='0' cellpadding='0' border='0' width='100%' height='25'><tr>
		...
		<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b>Total: <font class='totprice'>$<%= for12x.format(new Double(price).doubleValue()) %></font>&nbsp;<%=currencyName%></b></td>
	</tr>
</table>
<%
		double tax_dollar=0;
//out.println("1TAX:"+tax_perc+"::line_cost:"+line_cost);
		if (!(tax_perc==null||tax_perc.equals("0"))){
		//tax_dollar=(grand_total1*Double.parseDouble(tax_perc))/100;
		tax_dollar=(totalLineItemcost*Double.parseDouble(tax_perc))/100;
//out.println("2");
...
//out.println("b");
			...
			out.println("<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b><font class='totprice'>$"+for12x.format(tax_dollar)+"</font>&nbsp;"+currencyName+"</b></td>");
			out.println("</tr>");
			out.println("</table>");
			out.println("<table class='"+border_color+"' cellspacing='0' cellpadding='0' border='0' width='100%' height='25'><tr>");
			out.println("<tr>");
			out.println("<td class='tableline_row mainbody' width='50%' valign='middle'><b>Material and Tax Only </b></td>");
			out.println("<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b>Total: <font class='totprice'>$"+for12x.format(grand_total1)+"</font>&nbsp;"+currencyName+"</b></td>");
			out.println("</tr>");
			out.println("</table>");
