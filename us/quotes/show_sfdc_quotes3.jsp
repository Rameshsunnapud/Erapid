...
String tot_cost_new="0";
String tot_cost="0";
double temp_cost=0;
ResultSet myResult1=stmt.executeQuery("select sum(cast(Extended_Price as float)) from csquotes where Order_no like '"+order_no+"%' and product_id='GE' and not block_id ='B_NOTES' ");
if (myResult1 !=null) {
	while (myResult1.next()){
		tot_cost = myResult1.getString(1);
	}
}
ResultSet rsxx=stmt.executeQuery("select extended_price from csquotes where Order_no like '"+order_no+"%' and product_id!='GE' ");
if(rsxx != null){
	while(rsxx.next()){
		if(rsxx.getString(1)!= null & rsxx.getString(1).trim().length()>0){
			temp_cost=temp_cost+new Double(rsxx.getString(1)).doubleValue();
		}
	}
}
rsxx.close();
...
//out.println("tot_cost value is: ---> "+tot_cost_new);
	...
	if (rs_project !=null) {
		   while (rs_project.next()) {
			...
			userIdValue=rs_project.getString("user_id");
			
		}
                   configuredPriceDB = configured_price;
	}
...
price1=""+(new Double(overage).doubleValue()+new Double(handling_cost).doubleValue()+new Double(freight_cost).doubleValue()+new Double(tot_cost_new).doubleValue());
...
/*
	...
	if(rsSf0 != null){
		while(rsSf0.next()){
			...
			//	out.println(opportunityName+":: opp name<BR>");
			//out.println(rsSf0.getString("Account_Customer__C")+"::: account customer c::select * from Quotes__c where name='"+salesForceNo+"'<BR>");
			sfdcAccount=rsSf0.getString("Account_Customer__C");
			//	out.println(rsSf0.getString("Contact_Customer__C")+"::: contact customer c<BR>");
			quoteSource=rsSf0.getString("Quote_Scource__c");
			projectId=rsSf0.getString("Project__c");
            psa_spec_section=rsSf0.getString("Spec_section__c");
            psa_alternate=rsSf0.getString("Alternate__c");
            psa_addenda=rsSf0.getString("Addenda__c");
            if(rsSf0.getString("Bid_date__c")!= null && rsSf0.getString("Bid_date__c").length()>=10){
			edate=rsSf0.getString("Bid_date__c").substring(0,10);
            }
            addSFDC=rsSf0.getString("add__c");
            deductSFDC=rsSf0.getString("deduct__c");
...
*/
	...
	//out.println(sfdcNotes+"::sfdcnotes<BR>");
	...
	if(rsSf != null){
		while(rsSf.next()){
			...
			if(rsSf.getString("Country__c")!=null && !rsSf.getString("Country__c").equals("null") && rsSf.getString("Country__c").trim().length()>0 && !rsSf.getString("Account_Name__c").equals("Builders Specialties Inc")){
			...
			if(rsSf.getString("Fax")!=null && !rsSf.getString("Fax").equals("null") && rsSf.getString("Fax").trim().length()>0 && !rsSf.getString("Account_Name__c").equals("Builders Specialties Inc")){
...
*/
			...
			while(rsSfAccount2.next()){
				if(rsSfAccount2.getString(1) != null && rsSfAccount2.getString(1).trim().length()>0){
					if(exchName.equals("CAD")||exchName.equals("CAN")){
						...
						decoAdd="<BR><b>Prepared for:</b><BR>"+rsSfAccount2.getString(1);
                                            }
					}
				}
			}
	...
	//if(opportunityName != null && opportunityName.trim().length()>0){
	//	repAddress=opportunityName+"<BR>"+repAddress;
	//}
	//out.println(repAddress+"::<BR>");
...
exclusions_free_text="";
	...
	if (!(product.equals("LVR")|product.equals("BV")|product.equals("GRILLE")|product.equals("ADS"))) {
		...
		double extra_charges=new Double(overage).doubleValue()+new Double(setup_cost).doubleValue()+new Double(handling_cost).doubleValue()+new Double(freight_cost).doubleValue();
		//grand_total=tot_sum1+extra_charges;
		//The below line is commented by praveen sannadi to fix TAC 61698 and added next to that commented line
                //grand_total=new Double(configured_price).doubleValue()+extra_charges;
				if(product.equals("GE"))
				{
				grand_total=new Double(tot_cost_new).doubleValue()+extra_charges;	
				}
				else{
					grand_total=new Double(configured_price).doubleValue()+extra_charges;
				}
				
		//out.println(grand_total+"::"+configured_price+"::"+temp_cost+"::"+tot_cost+"::"+setup_cost+"::"+handling_cost+"::"+freight_cost+"::<BR>");
...
//out.println(exchName+"::::");
		ResultSet rs_ads=stmt.executeQuery("select * from cs_ads_price_calc where order_no like '"+order_no+"' and model='global'");
		if(rs_ads != null){
			while(rs_ads.next()){
				if(exchName.equals("CAD")||exchName.equals("CAN")){
					grand_total=new Double(rs_ads.getString("canlinePrice")).doubleValue();
				}
				else{
					grand_total=new Double(rs_ads.getString("linePrice")).doubleValue();
				}
			}
		}
		rs_ads.close();
	...
	String rep_name="";
	String rep_email2="";
	ResultSet rs_reps = stmt.executeQuery("SELECT * FROM cs_reps where rep_no like '"+rep_no+"' and user_id like '"+userIdValue+"'");
	if (rs_reps !=null) {
		while (rs_reps.next()) {
			...
			//decoRep=decoRep+"<B>Phone:</b>"+rs_reps.getString("telephone")+"<BR>";
			//decoRep=decoRep+"<B>Fax: </b>"+rs_reps.getString("fax");
			decoRep="<b>e-mail: </b>"+rs_reps.getString("email");
			decoRep=decoRep+"<br>Rep Name:"+rs_reps.getString("rep_name")+"<BR>"+decoAdd;
		}
	}
	//dates
...
<html>
	<head>
		...
		<style type="text/css">
			...
			.maintitle { color:#333333; font-size:13pt; font-family:arial, helvetica; }
			.maintitle2 { color:#333333; font-size:6pt;font-family:arial, helvetica; }
			.subtitle { color:#333333; font-size:10pt; font-family:arial, helvetica; }
			.mainfooter { color:#666666; font-size:6pt; font-family:tahoma, arial, helvetica; }
			.mainbody { color:#333333; font-size:9pt; font-family:arial, verdana, helvetica; text-decoration:none; }
			.mainbody2 { color:#333333; font-size:8pt; font-family:arial, verdana, helvetica; text-decoration:none; }
			.mainbodyh { color:#333333; font-size:8.5pt;font-family:verdana, arial, helvetica; text-decoration:none; }
			.maindesc { color:#333333; font-size:16pt; font-family:arial, verdana, helvetica; text-decoration:none; }
			.schedule { color:#000000; font-size:7.5pt; font-family:arial, verdana, helvetica; text-decoration:none; }
...
if(product.endsWith("EFS")){
	border_color="tableline_ejc";
	//out.println("Test by Java"+sections);
	%>
	<%@ include file="quote_template_body_efs_sfdc.jsp"%>
	<%
}
else if(product.endsWith("EJC")){
	border_color="tableline_ejc";
	//out.println(sections);
	%>
	<%@ include file="quote_template_body_ejc_sfdc.jsp"%>
	<%
}
else if(product.equals("GE")){
				border_color="tableline_ejc";
	%>
	<%@ include file="quote_template_body_ge.jsp"%>
	<%@ include file="ge_iwp_template.jsp"%>
	<%@ include file="quote_template_body_efs.jsp"%>
	<%
		}
if(sections<=1||product.equals("EFS")){
//out.println("2");
