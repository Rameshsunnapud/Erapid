...
exchRate=rs_project.getDouble("exch_rate");
			...
			contactId=rs_project.getString("contactId");
...
ResultSet rs_est=stmt.executeQuery("select rep_name,group_id from cs_reps where rep_no='"+rep_no+"'");
	...
	{
	//State name changes to State code Project State
	String projectStateCodeADS="";

	if(project_state!=null && project_state!="")
	{
	ResultSet rs_state_codes=stmt.executeQuery("select * from cs_state_codes where state_name='"+project_state+"'");
	if(rs_state_codes != null){
		while(rs_state_codes.next()){
			project_state=rs_state_codes.getString("state_code");
		}
	}
	rs_state_codes.close();
	}
	
	//State name changes to State code Arch state
				if(Arch_loc==null){
				Arch_loc="";
				}
	...
	String archStateCodeADS="";
	//out.println("State Test "+archState);
	if(archState!=null && archState!="")
	{
	ResultSet rs_state_codes_arch=stmt.executeQuery("select * from cs_state_codes where state_name='"+archState.trim()+"'");
	if(rs_state_codes_arch != null){
		while(rs_state_codes_arch.next()){
			archStateCodeADS=rs_state_codes_arch.getString("state_code");
		}
	}
	rs_state_codes_arch.close();
	}
	Arch_loc=archLoc+", "+archStateCodeADS;
	//out.println("State 2 "+archStateCodeADS);
	}
...
/*
	...
	if(rsSf0 != null){
		while(rsSf0.next()){
			...
			psa_rsm=rsSf0.getString("Regional_Sales_Manager__c");
	...
	rsSf0.close();
	ResultSet rsSf2x=stmt_sfdc.executeQuery("select Specification_Level__c,Competition_Specified__c,name from C_S_Opportunity__c where id='"+opportunityId+"'");
	if(rsSf2x != null){
		while(rsSf2x.next()){
			opportunityId2=rsSf2x.getString(3);
		}
	}
	rsSf2x.close();
...
*/
	...
	if(quoteSource==null || quoteSource.equals("null")){
		quoteSource="";
	}
	...
	if(rsSf != null){
		while(rsSf.next()){
			sfdcAccountId=rsSf.getString("AccountId");
                        //out.println(sfdcAccountId);
			if(!group_id.startsWith("Decolink")&&rsSf.getString("name")!=null && !rsSf.getString("name").equals("null") && rsSf.getString("name").trim().length()>0){
			...
			if(rsSf.getString("Country__c")!=null && !rsSf.getString("Country__c").equals("null") && rsSf.getString("Country__c").trim().length()>0 && !rsSf.getString("Account_Name__c").equals("Builders Specialties Inc")){
				repAddress=repAddress+"<BR>"+rsSf.getString("Country__c")+" ";
			}
			...
			if(!group_id.startsWith("Decolink")&&rsSf.getString("Email")!=null && !rsSf.getString("Email").equals("null") && rsSf.getString("Email").trim().length()>0){
				repAddress=repAddress+"Email:"+rsSf.getString("Email")+"<BR>";
			}
			if(group_id.startsWith("Decolink")&&rsSf.getString("name")!=null && !rsSf.getString("name").equals("null") && rsSf.getString("name").trim().length()>0){
				repAddress=repAddress+"Attn: "+rsSf.getString("name")+"<BR>";
			}
		}
	}
	rsSf.close();
	...
	if(sfdcAccountId != null && sfdcAccountId.trim().length()>0){
		ResultSet rsSfAccount=stmt_sfdc.executeQuery("select name,Rep_Agent_no__c from account where id='"+sfdcAccountId+"'");
		if(rsSfAccount != null){
			while(rsSfAccount.next()){
				repAddress=rsSfAccount.getString(1)+"<BR>"+repAddress;
				if(rsSfAccount.getString(2)!=null&&rsSfAccount.getString(2).trim().length()>0){
					repAddress=rsSfAccount.getString(2)+" - "+repAddress;
				}
			}
		}
		rsSfAccount.close();
...
if(group_id.startsWith("Decolink")&&(contactId==null||contactId.trim().length()==0||contactId.equals("null"))){
							repAddress=rsSfAccount2.getString(1)+"<BR>";
							repAddress=repAddress+rsSfAccount2.getString(2)+"<BR>";
							repAddress=repAddress+rsSfAccount2.getString(3)+","+rsSfAccount2.getString(4)+" "+rsSfAccount2.getString(5)+" "+rsSfAccount2.getString(6);
    ...
    if(!(type.equals("3")||type.equals("4"))){
		...
		rsSfAccount2.close();
		if(exchName.equals("CAD")||exchName.equals("CAN")){
			ResultSet rsSf2=stmt_sfdc.executeQuery("select * from contact where id='"+contactId+"'");
			if(rsSf2 != null){
				while(rsSf2.next()){
					if(rsSf2.getString("name")!=null && rsSf2.getString("name").trim().length()>0){
						canCustomer=canCustomer+"<BR>Attn: "+rsSf2.getString("name");
		}
					}
		}
		rsSf2.close();
...
if(product.endsWith("ADS")){
	border_color="tableline_ejc";
	//out.println(sections);
	%>
	<%@ include file="quote_template_body_ads_sfdc.jsp"%>
	<%
}
if(product.endsWith("IWP")){
	...
	//out.println("---------------------->>");
	%>
	<%@ include file="quote_template_body_iwp_sfdc.jsp"%>
	<%
