...
String group_id= request.getParameter("group_id");//for decding which group they belong to
%>
...
if (!(order_no==null)){
	...
	for(int i=0; i<distinctModels.size(); i++){
		...
		if(note28.indexOf("*"+distinctModels.elementAt(i).toString().replaceAll("-","")+"*")>=0){
            ...
            qualifying_notes=qualifying_notes+"92,";
            qual_count++;
        }
	//out.println(qualifying_notes+"::<BR>");
//out.println(overage+"::"+handling_cost+"::"+freight_cost);
	...
	if (rs_psa_quotes !=null) {
		while (rs_psa_quotes.next()) {
			//	out.println("HERE");
				...
				quote_region=rs_psa_quotes.getString("quote_region");
				quote_source=rs_psa_quotes.getString("quote_source");
				//out.println(quote_source+"::"+QuoteID+"::"+quote_region+"::<BR>");
			//out.println("psa"+psa_alternate+"s"+psa_addenda);
		}
	}
	rs_psa_quotes.close();
	if(quote_region==null){
			quote_region="";
	}
	if(quote_source==null){
			quote_source="";
	}
	ResultSet rsRegion=stmt_psa.executeQuery("select lookup_text from dba.user_lookup where lookup_id='"+quote_source+"'");
	if(rsRegion != null){
		while(rsRegion.next()){
			quote_region=rsRegion.getString(1);
		}
	}
	rsRegion.close();
	if(quote_region==null){
	...
	ResultSet rs_est=stmt.executeQuery("select rep_name from cs_reps where user_id='"+psa_estimator+"'");
	if(rs_est != null){
		while(rs_est.next()){
			psa_estimator=rs_est.getString(1);
		}
	}
	rs_est.close();
	stmts.close();
...
if(product.endsWith("LVR")){
	...
	%>
	<%@ include file="quote_template_body_lvr_psa.jsp"%>
	<%
}
...
out.println("No eRapid quote # is attached to this PSA quote");
}
stmt.close();
myConn.close();
	%>
