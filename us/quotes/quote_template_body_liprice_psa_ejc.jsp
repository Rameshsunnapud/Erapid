...
if(sections<=0){
	...
	for (int n=0;n<k;n++){
			...
			if ((bcount%2)==1){bgcolor="";}else {bgcolor="";	}
			 ...
			 String tot_sumx=""+(new Double(tot_sum).doubleValue()-new Double(overage).doubleValue()-new Double(freight_cost).doubleValue()-new Double(handling_cost).doubleValue());
...
if(overageSec.elementAt(ye).toString() != null && overageSec.elementAt(ye).toString().trim().length()>0){
	...
	price=""+for1.format(totQuotePrice);
