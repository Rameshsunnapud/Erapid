...
<table border='0' width='100%' class='table_thin_borders' style='border-collapse: collapse;' cellspacing='0' cellpadding='3'>
	...
	<table border='1' width='100%' class='table_thin_borders' style='border-collapse: collapse;' cellspacing='0' cellpadding='3'>
		<%
		      ...
		      // Spec Scetion added by Yasoda,GSO
		      if(! ((psa_spec_section==null)||((psa_spec_section.trim()).length()<0)) )  {
			 String[] specSections = psa_spec_section.split("\\r?\\n");
			 String tempSpecSection="";
			      for (String specSection : specSections) {
			      tempSpecSection=tempSpecSection+specSection+"<br>";
                               }
			psa_spec_section = tempSpecSection;
			}
			 // Alternate added by Yasoda,GSO
			 if(! ((psa_alternate==null)||((psa_alternate.trim()).length()<0)) )  {
			    String[] alternates = psa_alternate.split("\\r?\\n");
		            String tempAlternate="";
		            for (String alternate : alternates) {
			    tempAlternate=tempAlternate+alternate+"<br>";
			     }
			psa_alternate = tempAlternate;
			}
			 // Addenda added by Yasoda,GSO
			 if(! ((psa_addenda==null)||((psa_addenda.trim()).length()<0)) )  {
			    String[] addendas = psa_addenda.split("\\r?\\n");
		            String tempAddenda="";
		            for (String addenda : addendas) {
			    tempAddenda=tempAddenda+addenda+"<br>";
			     }
			psa_addenda = tempAddenda;			
			}
