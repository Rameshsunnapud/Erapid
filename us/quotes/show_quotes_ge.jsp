...
//out.println(overage+"::"+handling_cost+"::"+freight_cost+"::"+tot_cost_new);
...
String qlf_notes="";
 ...
 //out.println(configured_price+"::"+taxtotal);
...
ResultSet rsUser=stmt.executeQuery("select group_id,rep_name from cs_reps where user_id='"+creator_user_id+"'");
if(rsUser != null){
	while(rsUser.next()){
		...
		//out.println(rsUser.getString(1)+":::");
			...
			prepared_by=rsUser.getString("rep_name");
...
prepared_by_email= rs_reps.getString("email");
}
}
...
if(product.equals("GE")){
	//out.println("<BR><BR>GE<BR><BR>");
		...
		<%
			...
			else if(product.equals("SLM")){
					//out.println("<BR><BR>SLM<BR><BR>");
			//out.println("<BR><BR>HEREx<BR><BR>");
				border_color="tableline_iwp";
		%>
		<%//@ include file="quote_template_body_slm.jsp"%>
		<%
			}
			//out.println("HERE<BR>");
		//out.println(section_page+"::"+sections+ "<BR>");

			//if(((section_page==null||section_page.equals("0"))&&sections<2)||(section_page.equals("1")&&sections==0)){
