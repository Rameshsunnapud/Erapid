...
//out.println(section_page+"::"+sections);
	...
	if(product.equals("EJC")){
		...
		<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b>Total: <font class='totprice'><%= price %></font>&nbsp;<%=currencyName%></b></td>
            ...
            <td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b><font class='totprice'><%=for1.format(new Double(inittotal).doubleValue())  %>.00</font>&nbsp;<%=currencyName%></b></td>
            </tr>
            <%
		...
		<tr>
			...
			<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b><font class='totprice'>$<%=for9.format(temphandling_cost)%></font>&nbsp;<%=currencyName%></b></td>
		</tr>
		<%
            }
		...
		<tr>
			...
			<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b>Total: <font class='totprice'><%= for1.format(new Double(tempprice).doubleValue())%>.00</font>&nbsp;<%=currencyName%></b></td>
		</tr>
		<%
            }

            if(canType.equals("CAD")){
                out.println("<tr><td class='tableline_row mainbody' colspan='2' valign='middle'>F.C.A PLANT, FREIGHT ALLOWED EXCLUDING TAX.</td></tr>");
            }
            ...
            if(price.indexOf("$")<0&& price.indexOf(",")<0){
                price= "$"+for2ge.format(new Double(price).doubleValue());
            }
            
            //GCP-498
            //out.println("Subtotal (configured price) "+configuredPriceDB);            
           // out.println("freight_cost "+freight_cost);
            //out.println("taxZip "+tax_zip);
            double subTot = 0.0;
            double freight = 0.0;
            double taxforZip = 0.0;            
            double grandTotal = 0.0;
            
            if(null!= configuredPriceDB && !configuredPriceDB.isEmpty()){
            
                subTot = subTot + new Double(configuredPriceDB).doubleValue();
                
                if(null!= overage && !overage.isEmpty()){
                  subTot = subTot + new Double(overage).doubleValue();
                } 
                if(null!= handling_cost && !handling_cost.isEmpty()){
                  subTot = subTot + new Double(handling_cost).doubleValue();
                } 
                if(null!= setup_cost && !setup_cost.isEmpty()){
                  subTot = subTot + new Double(setup_cost).doubleValue();
                } 
                             
           		grandTotal = grandTotal + subTot;
            }
            if(null!= freight_cost && !freight_cost.isEmpty()){                        
                freight =   new Double(freight_cost).doubleValue();                  
           		grandTotal = grandTotal + freight;
            }
               
           // if(null!= tax_zip && !tax_zip.isEmpty()){
             	// taxforZip =   new Double(tax_zip).doubleValue();       
           		// grandTotal = grandTotal + taxforZip;
           // }  
            
            double priceVal_01 = 0;             
			if (price != null || price.trim() != "") {
				priceVal_01 = Double.parseDouble(price.replace("$","").replaceAll(",",""));
			}
			double tax_dollar_01=0;
			if(tax_perc != null && tax_perc.trim().length()>0){
				tax_dollar_01 = (priceVal_01*Double.parseDouble(tax_perc))/100;		
				grandTotal = grandTotal + tax_dollar_01;		
			}
                
           //out.println("currencyName "+currencyName);            
         //   out.println("grandTotal is "+grandTotal);
            %>
            <table class='<%= border_color %>' cellspacing='0' cellpadding='0' border='0' width='100%' height='25'><tr>
		<td class='tableline_row mainbody' width='50%' valign='middle'><b>Material Furnished Only </b></td>
		<!--  GCP-498 -->
		<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b>Sub Total: <font class='totprice'><%=for0.format(subTot)%></font>&nbsp;<%=currencyName%></b></td>
		</tr>
		<tr>	
		<td class='tableline_row mainbody' width='50%' valign='middle'><b> </b></td>
		<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b>Freight: <font class='totprice'><%=for0.format(freight)%></font>&nbsp;<%=currencyName%></b></td>
		</tr>
		<tr>
		<td class='tableline_row mainbody' width='50%' valign='middle'><b> </b></td>
		<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b>Tax: <font class='totprice'><%=for0.format(tax_dollar_01)%></font>&nbsp;<%=currencyName%></b></td>
		</tr>
		<tr>
		<td class='tableline_row mainbody' width='50%' valign='middle'><b> </b></td>
		<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b>Grand Total: <font class='totprice'><%=for0.format(grandTotal)%></font>&nbsp;<%=currencyName%></b></td>
		</tr>
            <%
	}
        ...
        double priceVal = 0;
		if (price != null || price.trim() != "") {
			priceVal = Double.parseDouble(price.replace("$","").replaceAll(",",""));
		}
		double tax_dollar=0;
		if(tax_perc != null && tax_perc.trim().length()>0){
			tax_dollar=(priceVal*Double.parseDouble(tax_perc))/100;
		}
		grand_total=priceVal+tax_dollar;
	//out.println("Testttttttt 95 "+grand_total);
...
/*
		...
		<jsp:include page="summary_tax.jsp" flush="true">
			...
			<jsp:param name="totprice_dis" value="<%= grand_total_for_summary_tax_input%>"/>
...
*/
	...
	if(tax_perc != null && !tax_perc.equals("0") && !tax_perc.equals("0.0") && tax_perc.trim().length()>0){
		...
		if(freight_cost != null && freight_cost.trim().length()>0 && new Double(freight_cost).doubleValue()>0){
			...
			<table class='tableline_iwp' cellspacing='0' cellpadding='0' border='0' width='100%' height='25'>
				<td class='tableline_row mainbody' width='50%' valign='middle'><b>Total Cost:</b></td>
				<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b> <font class='totprice'>$<%= for2ge.format(new Double((price1)).doubleValue()-new Double(freight_cost).doubleValue()) %></font>&nbsp;<%=currencyName%></b></td>

			</table>

			<table class='tableline_iwp' cellspacing='0' cellpadding='0' border='0' width='100%' height='25'>
				<tr>
					<td class='tableline_row mainbody' width
					    ='50%' valign='middle'><b>Freight:</b></td>
					<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b> <font class='totprice'>$<%= for2ge.format(new Double(freight_cost).doubleValue()) %></font>&nbsp;<%=currencyName%></b></td>
				</tr>
			...
			<table class='tableline_iwp' cellspacing='0' cellpadding='0' border='0' width='100%' height='25'>
				<tr>
					<td class='tableline_row mainbody' width='50%' valign='middle'>Total Cost: </td>
					<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b><font class='totprice'><%= for2ge.format(new Double((price1)).doubleValue()) %></font>&nbsp;<%=currencyName%></b></td>
				</tr>
				<tr><td class='tableline_row mainbody' colspan='2' valign='middle'>Price is FOB plant. </td></tr>
			</table>
			<%
		...
		if(freight_cost != null && freight_cost.trim().length()>0 && new Double(freight_cost).doubleValue()>0){
			...
			<table class='tableline_iwp' cellspacing='0' cellpadding='0' border='0' width='100%' height='25'>
				<tr>
					...
					<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b><font class='totprice'>$<%=for2ge.format(grand_total)%></font>&nbsp;<%=currencyName%></b></td>
				</tr>
				<tr><td class='tableline_row mainbody' colspan='2' valign='middle'>Price is FOB plant. </td></tr>
			</table>
			<%
			...
			<table class='tableline_iwp' cellspacing='0' cellpadding='0' border='0' width='100%' height='25'>
				<tr>
					<td class='tableline_row mainbody' width='50%' valign='middle'>&nbsp;</td>
					<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b>Total: <font class='totprice'><%= for2ge.format(new Double((price1)).doubleValue()) %></font>&nbsp;<%=currencyName%></b></td>
				</tr>
				<tr><td class='tableline_row mainbody' colspan='2' valign='middle'>Price is FOB plant. </td></tr>
			</table>
			<%
	...
	if(tax_perc != null && !tax_perc.equals("0") && !tax_perc.equals("0.0") && tax_perc.trim().length()>0){
		taxtotal=new Double((price1)).doubleValue()-new Double(freight_cost).doubleValue();
		//out.println("Price1 here "+taxtotal);
		...
		if(overage != null && overage.trim().length()>0){
			...
			<table border='0' width='100%'>
				...
				if(!group_id.startsWith("Internatio")&!type.equals("5")&!type.equals("6")) {
					...
					else if(product.endsWith("ADS")|product.endsWith("LVR")|product.endsWith("FSM")|product.equals("BV")|product.endsWith("GRILLE")|product.endsWith("GCP") ||canType.equals("CAD")){
		...
		*/
			...
			out.println("<td align='left' valign='bottom' nowrap class='maindesc'>On-Site Phone No</td><td valign='bottom' class='mainbody'>................................................</td>");
		} else {
			out.println("<td align='left' valign='bottom' nowrap class='maindesc'>Site Phone No</td><td valign='bottom' class='mainbody'>................................................</td>");
		} %>
				...
				<tr><td align='left' valign='bottom' nowrap class='maindesc'>Customer Signature: </td>
					<td valign='bottom' class='mainbody'>................................................</td>
					<%		if (product.equals("GE")){
								out.println("<td align='left' valign='bottom' nowrap class='maindesc'>On-Site Contact Name:</td><td valign='bottom' class='mainbodyh'>................................................</td>");
							} else {
			...
			<table cellspacing='0' cellpadding='0' border='0' width='100%'>
				...
				<tr><td class='mainfooter'>
						...
						%>
						<br>1.	The Buyer/Customer agrees to all CS Terms and Conditions when they place their purchase order against this quote.
						<br>2.	Payment Terms: Payment in advance or Letter of Credit (LOC) prior to production; all CS invoices must be paid in US Dollars.
						<br>3.	All banking, consulate, government requirements and/or inspection fees will be Buyer/Customer's responsibility.
						<br>4.  Prices are firm against escalation for 90 days from date of this quote, and for shipment within 6 months thereafter. Orders shipped beyond shall be subject to escalation of 0.5% per month or partial month thereafter and invoiced at time of shipment.
						...
						<br>9.	All CS orders will be shipped EX Works (EXW), or Free Carrier (FCA) Buyer's Forwarder - Continental U.S. Port (to be determined in final quote), referencing INCOTERMS 2010.
						<br>10.	All duties, fees, taxes or other costs associated with customs clearance or import duties are the sole responsibility of the Buyer/Customer. CS will issue Certificates of Origin for eligible products only.
						...
						<BR>13. For projects located outside the U.S. Construction Specialties requires that all freight forwarders who handle CS product be C-TPAT certified. Please contact your local representative for more information and a list of available C-TPAT carriers in your area.
	...
	//if(product.equals("GE")) {out.println("Construction Specialties");}
	//else {out.println("CS");}
						--%> 
