...
if(sections<=0){
	...
	for(int i=0; i<desc.size(); i++){
		...
		//}
                /*
		//TAC # 69952, Fixed by Srinivasa. Added b_frames_sp block_id.
		ResultSet rsframes=stmt.executeQuery("select count(*) from csquotes where order_no ='"+order_no+"' and line_no ='"+line_no.elementAt(i).toString()+"' and (block_id='b_frames' or block_id='b_frames_sp' or block_id='a_foprices')");
		if(rsframes != null){
			while(rsframes.next()){
...
out.println("<tr>");
	...
	out.println("<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b>Total: <font class='totprice'>"+for1.format(grand_total1x)+"</font>&nbsp;"+currencyName+"</b></td>");
	out.println("</tr>");
	out.println("</table>");
