<hr>
<%
	DecimalFormat dfor12=new DecimalFormat("#.##");
	dfor12.setMaximumFractionDigits(0);
	dfor12.setMinimumFractionDigits(0);
	double cQty=0;
	double cQtySec=0;
	double cSqftTemp=0;
	String cWindload="";
	...
	if(rsCosting != null){
		while(rsCosting.next()){
			cQty=cQty+rsCosting.getDouble("qty");
			cQtySec=cQtySec+rsCosting.getDouble("qty")*rsCosting.getDouble("stile_code");
			if(cWindload.indexOf("#"+rsCosting.getString("clad_code")+"#")<0){
				cWindload=cWindload+"#"+rsCosting.getString("clad_code")+"#";
			}
	...
	cSqft = (int) cSqftTemp;
	rsCosting.close();
	cWindload=cWindload.replaceAll("##",",");
	cWindload=cWindload.replaceAll("#","");

%>
	<table width="100%">
		<tr>
			<td width="50%"><label>Spec. Section:</label>&nbsp;<%=section%></td>
			<td width="50%">Wind load PSF:&nbsp;<%=cWindload%></td>
		</tr>
		<tr>
			<td width="50%"><label>Alternate:</label>&nbsp;None</td>
			<td width="50%">Number of Louvers:&nbsp;<%=dfor12.format(cQty)%></td>
		</tr>
		<tr>
			<td width="50%"><label>Addenda:</label>&nbsp;None</td>
			<td width="50%">Number of Sections:&nbsp;<%=dfor12.format(cQtySec)%></td>
		</tr>
		<tr>
			<td colspan="2" align="center"><div class="styletr">&nbsp;<div></td>
		</tr>
	...
	ResultSet rs_cscosting = stmt.executeQuery("select * from cs_costing where order_no='"+order_no+"' and model not in ('8615F') order by cast(item_no as integer)");
		if (rs_cscosting !=null) {
		   while (rs_cscosting.next()) {
			...
			if(rs_cscosting.getString("clad_code") != null){
				wind_load.addElement(rs_cscosting.getString("clad_code"));
			}
			else{
				wind_load.addElement("");
			}
			t_costing++;
			}
		}
		rs_cscosting.close();
int t_group=0;
	ResultSet rs_cscosting_group = stmt.executeQuery("SELECT MODEL,FINISH,SCREEN,COUNT(*)as v,min(cast(item_no as integer)) from cs_costing where order_no='"+order_no+"' and model not in ('8615F') GROUP BY MODEL,FINISH,SCREEN order by 5");
...
//out.println(block_id.elementAt(v).toString()+"::"+desc.elementAt(v).toString()+"::::<BR>");

										if ((block_id.elementAt(v).toString()).equals("A_PRICING")&&desc.elementAt(v)!= null&&desc.elementAt(v).toString().trim().length()>0){
											out.println("<tr><td class='data2'><p><b>Model:&nbsp;</b>"+desc.elementAt(v).toString()+"</p></td></tr>");
										}
							...
							if (rs_csfinish !=null) {
							   while (rs_csfinish.next()) {
									...
									if (finish_desc != null) {
										finish_desc = finish_desc.replaceFirst("finish:","");
										finish_desc = finish_desc.replaceFirst("FINISH:","");
									} 
							   }
							}
							...
							if (rs_csscreen !=null) {
								while (rs_csscreen.next()) {
									...
									if (screen_desc != null) {
										screen_desc = screen_desc.replaceFirst("screen:","");
										screen_desc = screen_desc.replaceFirst("SCREEN:","");
										//out.println("aaa"+screen_desc);
									}
								}
							}
							rs_csscreen.close();

							out.println("<tr><td class='data2'><p><b>Screen:&nbsp;<b>"+screen_desc+"</p></td></tr>");
							out.println("<tr><td class='data2'><p><b>Finish:&nbsp;</b>"+finish_desc+"</p></td></tr>");
							out.println("<tr><td>&nbsp;</td></tr>");
							out.println("</table>");
							out.println("<table width='100%' cellspacing='0' cellpadding='0' border='0' class='tab_body_info'>");
							out.println("<tr height='20'>");
							out.println("<th width='12%'>Mark</th>");
							out.println("<th width='12%'>Qty</th>");
							out.println("<th width='13%'>Size (W x H)</th>");
							out.println("<th width='13%'>Shape</th>");
							out.println("<th width='13%'>Blank Off</th>");
							out.println("<th width='13%'>Sills</th>");
							out.println("<th width='12%'>Snap on Closure</th>");
							out.println("<th width='12%'>Wind Load</th>");
							out.println("</tr>");
							out.println("<tr>");
							out.println("<td valign='top' nowrap>"+mark.elementAt(n).toString()+"</td>");
							out.println("<td valign='top' nowrap>"+for12.format(new Double(qty.elementAt(n).toString()).doubleValue())+"</td>");
							out.println("<td valign='top' nowrap>"+width.elementAt(n).toString()+" x "+height.elementAt(n).toString()+"</td>");
	...
	//						if(shape.trim().length()<=0){shape=" ";}
							out.println("<td valign='top' nowrap>"+shape+"</td>");
							out.println("<td valign='top' nowrap>"+blank_off+"</td>");
							out.println("<td valign='top' nowrap>"+sills+"</td>");
							out.println("<td valign='top' nowrap>"+snap_ons+"</td>");
							out.println("<td valign='top' nowrap>"+wind_load.elementAt(n).toString()+"</td>");
							out.println("</tr>");
...
//							if(shape.trim().length()<=0){shape=" ";}
							out.println("<tr>");
							out.println("<td valign='top' width='5%' nowrap>"+mark.elementAt(n).toString()+"</td>");
							out.println("<td valign='top' nowrap width='5%'>"+for12.format(new Double(qty.elementAt(n).toString()).doubleValue())+"</td>");
							out.println("<td valign='top' nowrap width='7%' nowrap>"+width.elementAt(n).toString()+" x "+height.elementAt(n).toString()+"</td>");
							out.println("<td valign='top' width='5%' nowrap>"+shape+"</td>");
							out.println("<td valign='top' width='5%' nowrap>"+blank_off+"</td>");
							out.println("<td valign='top' width='5%' nowrap>"+sills+"</td>");
							out.println("<td valign='top' width='5%' nowrap>"+snap_ons+"</td>");
							out.println("<td valign='top' nowrap>"+wind_load.elementAt(n).toString()+"</td>");
							out.println("</tr>");
							}
