...
String freight_cost="";String projCur="";String overage="";String handling_cost="";String project_type="";String carriage_charge="";String rep_notes=""; String tax_perc=""; String creator_user_id="";
        ...
        while (rs_project.next()) {
			...
			//   out.println(configured_price);
				...
				if(product.equals("LVR") || product.equals("GRILLE")){
					price=rs_project.getString("configured_price");
				}
                                                                }
                                                            }
	...
	if(qualifying_notes_free_text==null||qualifying_notes_free_text.equals("null")){
		qualifying_notes_free_text="";
	}
	if(exclusions_free_text==null||exclusions_free_text.equals("null")){
		exclusions_free_text="";
	}
	if(free_text==null||free_text.equals("null")){
		free_text="";
	}
	if(pricing_options==null||pricing_options.equals("null")){
		pricing_options="";
	}
	if(pricing_options_free_text==null||pricing_options_free_text.equals("null")){
...
if(product.equals("LVR") || product.equals("GRILLE")){
	price=""+(new Double(price).doubleValue()+new Double(overage).doubleValue()+new Double(setup_cost).doubleValue()+new Double(handling_cost).doubleValue()+new Double(freight_cost).doubleValue());
...
grand_total2=duty+tot_sum1+new Double(overage).doubleValue()+new Double(setup_cost).doubleValue()+new Double(freight_cost).doubleValue()+new Double(handling_cost).doubleValue();
...
customerCountry=rs_customers.getString("country");
	...
	if(sfdcAccount != null && sfdcAccount.trim().length()>0){
		ResultSet rsSfAccount2=stmt_sfdc.executeQuery("select name,street_Name__c,City_or_Town__c,State_Province__c ,Postal_Code_or_Zip_Code__c,Country__c,fax,phone from account where id='"+sfdcAccount+"'");
		if(rsSfAccount2 != null){
			while(rsSfAccount2.next()){
				if(rsSfAccount2.getString(1) != null && rsSfAccount2.getString(1).trim().length()>0){
						...
						cust_addr1=rsSfAccount2.getString(2);
						city=rsSfAccount2.getString(3);
						state=rsSfAccount2.getString(4);
						zip_code=rsSfAccount2.getString(5);
						customerCountry=rsSfAccount2.getString(6);
                                        cust_phone= rsSfAccount2.getString("phone");
//out.println(cust_phone+":: cust phone");
                                        cust_fax= rsSfAccount2.getString("fax");

				}
			}
		}
		rsSfAccount2.close();
		ResultSet rsSFQuotes=stmt_sfdc.executeQuery("select Customer_Contact_Name__c,qualification_notes__c,Exclusion_Notes__c from quotes__c where erapid_quote_no__C='"+order_no+"' ");
		if(rsSFQuotes != null){
			while(rsSFQuotes.next()){
				cust_contact_name= rsSFQuotes.getString(1);
				//qualifying_notes_free_text=rsSFQuotes.getString(2);
				//exclusions_free_text=rsSFQuotes.getString(3);
			}
		}
		rsSFQuotes.close();
		if(qualifying_notes_free_text==null||qualifying_notes_free_text.equals("null")){
...
String rep_telephone="";String rep_fax="";String rep_email=""; String rep_name="";String rccode="";String repNumber = "";
//out.println(rep_no);
if(rep_no.length()>2){
...
repNumber = rs_reps.getString("rep_no");
                                               }
                                       }
...
<html>
	<head>
		...
		<% if(product.equals("LVR")){ %>
			<link rel='stylesheet' href='greenbelt_lvr.css' type='text/css' /></head>
		<%}%>
	...
	if(section_page==null||section_page.equals("0")||sections<=0){//cheking to print different pages
		if(customerCountry.equals("CANADA")){
					 //out.println("HERE");
		%>
		<%@ include file="quote_template_headerCAN.jsp"%>
		<%@ include file="quote_template_topCAN.jsp"%>

		<%

		}
		  else{
		  //out.println("HERE2"+product);
				if(product.equals("LVR")){
				%>
				<%@ include file="quote_template_header_gb_lvr.jsp"%>
				<%@ include file="quote_template_top_gb_lvr.jsp"%>
				<% 
				} else {
				%>
  ...
  //out.println("<BR><BR>EJC Here<BR><BR>");
		%>
  ...
  //out.println("<BR><BR>HEREx<BR><BR>");
		...
		<%
			   ...
			   else if(product.endsWith("LVR")){
					 border_color="tableline_ejc";
		%>
		<%@ include file="quote_template_body_gb_lvr.jsp"%>
		<%	}
			
			   else if(product.endsWith("GRILLE")){
					 border_color="tableline_ejc";
		%>
		<%@ include file="quote_template_body_grille.jsp"%>
		<%	}
	...
	if(!(product.equals("IWP")&& sections>1)){
		 //out.println("if(!(product.equals("IWP")&& sections>1)){");
					 //out.println("HERE");
