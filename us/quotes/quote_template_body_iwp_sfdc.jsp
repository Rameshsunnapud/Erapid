...
if( (new Double(handling_cost)).doubleValue()>0 &&!group_id.startsWith("Decolink")){

					out.println("<tr>");
					if(group_id.startsWith("Decolink")){
						out.println("<td valign='top'  "+bodyString+">"+"Freight Surcharge."+"</td>");
					}
					else{
...
out.println("</tr>");
double sumX=0;
//out.println("select sum(cast(extended_price as numeric)) from csquotes where order_no='"+order_no+"' and ((Block_ID !='A_APRODUCT' and Block_ID like 'A%') or Block_id = 'C_OPTIONS' or block_id like 'b_charge%') and line_no in("+sect_group_lines.elementAt(ye).toString()+")<BR><BR>");

ResultSet rsSum=stmt.executeQuery("select extended_price from csquotes where order_no='"+order_no+"' and ((Block_ID !='A_APRODUCT' and Block_ID like 'A%') or Block_id = 'C_OPTIONS' or block_id like 'b_charge%') and line_no in("+sect_group_lines.elementAt(ye).toString()+")");
if(rsSum != null){
while(rsSum.next()){
	sumX=sumX+new Double(rsSum.getString(1)).doubleValue();
}
}
rsSum.close();
out.println(sumX);
...
<table>
	...
	if(qual_sect!=null&&qual_sect.trim().length()>0){
		...
		if("IWP".equals(product) && (exchName.equals("CAD") || group_id.contains("can"))){
			qualyfyingNotesQuery = "select description FROM cs_qlf_notes where product_id='"+product+"' and code in ("+qual_sect+") and code != '74' order by code ";
		}else{
			qualyfyingNotesQuery = "select description FROM cs_qlf_notes where product_id='"+product+"' and code in ("+qual_sect+") order by code ";
		}
		ResultSet cs_exc_notes1 = stmt.executeQuery(qualyfyingNotesQuery);
...
<table class='<%= border_color %>' cellspacing='0' cellpadding='0' border='0' width='100%' height='25'><tr>
		...
		<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b>Total: <font class='totprice'><%=for1.format(new Double(inittotal).doubleValue())  %></font>&nbsp;<%=currencyName%></b></td>
	</tr>
	<%
	...
	<tr>
		...
		<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b><font class='totprice'><%=for1.format(temphandling_cost)%></font>&nbsp;<%=currencyName%></b></td>
	</tr>
	<tr>
		<td class='tableline_row mainbody' width='50%' valign='middle'>&nbsp;</td>
		<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b>Total: <font class='totprice'><%=for1.format(grand_total) %></font>&nbsp;<%=currencyName%></b></td>
	</tr>
	<%
}
out.println("</table>");
