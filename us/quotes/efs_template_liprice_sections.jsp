...
if(sections<=1){
	...
	for(int ye=0;ye<sect_group.size();ye++){
		...
		//double overageX=0;
					//out.println("freightb:"+freight_cost+"sumx"+sumX+":::::"+sumTotal+" calculation : "+(sumX/sumTotal));
		...
		if(isSecHandling&&handlingSec.size()>0){
			if(handlingSec.elementAt(ye) != null && handlingSec.elementAt(ye).toString().trim().length()>0){
				...
				//out.println("total price "+handlingX);
			}
		}
		else{handlingX=new Double(handling_cost).doubleValue()*(sumX/sumTotal);}
		//out.println("total price 2 "+sumTotal);
		...
		if(isSecSetup&&setupSec.size()>0){
			if(setupSec.elementAt(ye) != null && setupSec.elementAt(ye).toString().trim().length()>0){
				setupX=new Double(setupSec.elementAt(ye).toString()).doubleValue();
					...
					//out.println(sumX+"freight:"+freightX);
		//out.println(overageX+" -> overage and setup -> "+handlingX);
		totprice=sumX+overageX+freightX+handlingX+setupX;
		//out.println(sumX+" total price "+totprice);
...
//out.println(price+"::"+grand_total3*10);
//grand_total3=new Double(price).doubleValue();
grand_total1=totprice;
grand_total3=Float.parseFloat(price);
//out.println("section_page "+totprice);
//out.println(section_page);
if(section_page==null||section_page.equals("1")){
out.println(section_page);
%>
...
<table class='<%= border_color %>' cellspacing='0' cellpadding='0' border='0' width='100%' height='25'><tr>
		                <%
				if((product.equals("EFS"))&&!(tax_perc==null||tax_perc.equals("0"))){
				%>
...
if (!(tax_perc==null||tax_perc.equals("0"))){
	...
	out.println("<td class='tableline_row mainbody' width='50%' align='right' valign='middle' nowrap><b>Total: <font class='totprice'>"+for1.format(grand_total1)+"</font>&nbsp;"+currencyName+"</b></td>");
	out.println("</tr>");
	out.println("</table>");
