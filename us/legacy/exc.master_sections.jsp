...
<!--
			...
			function initSelectedNotes( ){
				...
				var szNotes="";
				var lc=<%= pl %>;
				szNotes=self.opener.window.document.forms[0].q[lc-1].value;
				if(szNotes=="")
					return;
...
-->
	...
	<center>
		<form method="POST">
			<table>
				...
				Vector code=new Vector();Vector desc=new Vector();int count=0;
					    ResultSet rs = stmt.executeQuery("SELECT * FROM cs_qlf_notes where product_id like '"+pid+"' order by code");
					  while ( rs.next() ) {
					   code.addElement(rs.getString( 2 )) ;
					  desc.addElement(rs.getString( 3 )) ;
					   count++;
										   }
				if (count>0){
				%>
		...
		out.println("There are no standarad Qualifying Notes for <b>"+pid+"</b> .please contact the <b>"+pid+" </b>product team for more info");
		}
		rs.close();
		stmt.close();
		myConn.close();
