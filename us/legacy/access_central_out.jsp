...
String export=request.getParameter("exportx");
out.println(interco);
if(interco.toUpperCase().equals("Y") || interco.toUpperCase().equals("ON")){
	interco="Y";
}
else{
	interco="";
}
if(export.toUpperCase().equals("Y") || export.toUpperCase().equals("ON")){
	export="Y";
}
else{
	export="";
...
if(request.getParameter("isNotSec") != null && request.getParameter("isNotSec").equals("false")){
	...
	String secTemp="";
	ResultSet rsIsSec=stmt.executeQuery("Select section_group,sections from cs_quote_sections where order_no='"+order_no+"'");
	if(rsIsSec != null){
		while(rsIsSec.next()){
			secTemp=rsIsSec.getString(1);
			sectionsNo=rsIsSec.getString(2);
		}
	}
	//out.println(secTemp);
...
if(section == null || section.trim().length()<1 || section.toLowerCase().equals("null")){
	section="0";
}
String qtype = request.getParameter("qtype");
//String pInfCOST = request.getParameter("pInfCOST");
//String pInfSELLPRICE = request.getParameter("pInfSELLPRICE");
//String[] cost_c=request.getParameterValues("cost_c");
...
String exportCrate=request.getParameter("exportCrate");
...
out.println("<BR><BR>"+request.getParameter("pInfTOTAL")+"<BR><BR>");
...
double pInfKACOMMPERC= (new Double(request.getParameter("pInfKACOMMPERC")).doubleValue());
double pInfSTDCOMMPERC= (new Double(request.getParameter("pInfSTDCOMMPERC")).doubleValue());
double pInfKACOMMDOLLARS= (new Double(request.getParameter("pInfKACOMMDOLLARS")).doubleValue());
double pInfSTDCOMMDOLLARS= (new Double(request.getParameter("pInfSTDCOMMDOLLARS")).doubleValue());
...
String pInfCATCHALL_DESC1=request.getParameter("pInfCATCHALL_DESC1");
String pInfCATCHALL_DESC2=request.getParameter("pInfCATCHALL_DESC2");
String pInfCATCHALL_DESC3=request.getParameter("pInfCATCHALL_DESC3");
String pInfCATCHALL_DESC4=request.getParameter("pInfCATCHALL_DESC4");
String pInfCATCHALL_DESC5=request.getParameter("pInfCATCHALL_DESC5");
double pInfCATCHALL1=(new Double(request.getParameter("pInfCATCHALL1")).doubleValue());
double pInfCATCHALL2=(new Double(request.getParameter("pInfCATCHALL2")).doubleValue());
double pInfCATCHALL3=(new Double(request.getParameter("pInfCATCHALL3")).doubleValue());
double pInfCATCHALL4=(new Double(request.getParameter("pInfCATCHALL4")).doubleValue());
double pInfCATCHALL5=(new Double(request.getParameter("pInfCATCHALL5")).doubleValue());
double pInfCATCHALLWT1=(new Double(request.getParameter("pInfCATCHALLWT1")).doubleValue());
double pInfCATCHALLWT2=(new Double(request.getParameter("pInfCATCHALLWT2")).doubleValue());
double pInfCATCHALLWT3=(new Double(request.getParameter("pInfCATCHALLWT3")).doubleValue());
double pInfCATCHALLWT4=(new Double(request.getParameter("pInfCATCHALLWT4")).doubleValue());
double pInfCATCHALLWT5=(new Double(request.getParameter("pInfCATCHALLWT5")).doubleValue());
...
if(rs00!=null){
	    while (rs00.next() ){
			...
			//cost_c[rowsx]=cost_c[rows]+cur_cost;
...
//out.println("Qty="+qty_c[rowsx]+"<BR>");
		  ...
		  if (product_c[rowsx].equals("GRILLE")){
					    if (rowsx!=0){
							 for (int pp=0;pp<rowsx;pp++) {
								if (model_c[pp].equals(model_c[rowsx])) {
								    //cur_cost = 0;
								    }
								}
							}
						    // out.println("#"+rowsx+" is a "+model_c[rowsx]+" "+product_c[rowsx]+"; setup cost is "+cur_cost);
			  }
...
try{
	...
	if(sectionsNo!=null){
		if(sectionsNo.trim().length()>0){
			if(Integer.parseInt(sectionsNo)<2){
				del0="delete from cs_access_central where order_no='"+order_no+"' ";
			}
		}
	}
...
try{
	...
	updateAccess.setString(52,interco);
	updateAccess.setString(53,pInfCATCHALL_DESC1);
	updateAccess.setString(54,pInfCATCHALL_DESC2);
	updateAccess.setString(55,pInfCATCHALL_DESC3);
	updateAccess.setString(56,pInfCATCHALL_DESC4);
	updateAccess.setString(57,pInfCATCHALL_DESC5);
	updateAccess.setString(58,pInfCATCHALL1+"");
	updateAccess.setString(59,pInfCATCHALL2+"");
	updateAccess.setString(60,pInfCATCHALL3+"");
	updateAccess.setString(61,pInfCATCHALL4+"");
	updateAccess.setString(62,pInfCATCHALL5+"");
	updateAccess.setString(63,pInfCATCHALLWT1+"");
	updateAccess.setString(64,pInfCATCHALLWT2+"");
	updateAccess.setString(65,pInfCATCHALLWT3+"");
	updateAccess.setString(66,pInfCATCHALLWT4+"");
	updateAccess.setString(67,pInfCATCHALLWT5+"");
	updateAccess.setString(68,export+"");
	updateAccess.setString(69,exportCrate+"");
	updateAccess.setString(70,pInfKACOMMPERC+"");
	updateAccess.setString(71,pInfSTDCOMMPERC+"");
	updateAccess.setString(72,pInfKACOMMDOLLARS+"");
	updateAccess.setString(73,pInfSTDCOMMDOLLARS+"");
...
try{
	...
	if(isNotSec){
		...
		del2="delete from cs_quotesz where quote_no='"+order_no+"' and line_no in('"+section+"','0')";
	}
...
try{
out.println("<BR><BR>"+(long)pInfTOTAL+"::<BR>"+id4export+"::<BR>"+VR_WEIGHT+"::<BR> AFTer<BR>");
	...
	updateQuotesz.setString(5,(long)pInfTOTAL+"");//pinftotal
