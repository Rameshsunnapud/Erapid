...
public class QuoteHeaderBean{
	...
	public void setQType(String x){
		...
		try{
			...
			//logger.debug(query+"::: query<BR>");
			//logger.debug(orderDate+":: BEFORE");
			...
			if(rs1 != null){
				while(rs1.next()){
					...
					taxZip = rs1.getString("tax_zip");
					taxExempt = rs1.getString("tax_exempt");
					changeCustomer = rs1.getString("changeCustomer");
					changeArch = rs1.getString("changeArch");
					assignedRepNo = rs1.getString("assigned_rep_no");
					sfdc_QuoteId = rs1.getString("SFDC_QuoteId");
					//logger.debug("3");
				}
			}
			rs1.close();
			//logger.debug(taxZip+"::"+taxExempt+":: quoteheaderbean line 210");
			...
			if(assignedRepNo == null){
				assignedRepNo = "";
			}
			if(sfdc_QuoteId == null){
				sfdc_QuoteId = "";
			}
		}
		catch(Exception e){
			orderNo = "" + e;
			logger.debug("QuoteHeaderBean.setQType");
			logger.debug(e.getMessage());
			logger.debug("QuoteHeaderBean.setQType END");
	...
	public void setOrderNo(String x){
		...
		try{
			...
			rs1.close();
			//logger.debug(taxZip+"::"+taxExempt+":: quoteheaderbean line 546");
			
			//TAC: # 73401 : start			
			if(null!= productId && !productId.isEmpty() && productId.equals("GRILLE")){
				boolean flag =   isGivenNumberExist("23", qualifyingNotes);
				if(!flag){
					qualifyingNotes = addQlyForModels(stmt, orderNo, qualifyingNotes);
				}
			}			
			//TAC: # 73401 : end		
			query = "select email,user_id,group_id from cs_reps where rep_no_text='" + creatorId + "' and user_id = '" + createdBy + "'";
			...
			orderNo = "" + e;
			logger.debug("QuoteHeaderBean.setOrderNo");
			logger.debug(e.getMessage());
			logger.debug("QuoteHeaderBean.setOrderNo END");
			//System.out.println(e);
		}
		finally{
			try{
	...
	public String getSFDC_QuoteId(String orderNo){
		...
		try{
			...
			stmt.executeUpdate("set ANSI_warnings off");
			String sfdc_QuoteId = "";
			ResultSet rs_find = stmt.executeQuery("SELECT SFDC_QuoteId FROM cs_project where order_no like '" + orderNo + "'");
			if(rs_find != null){
				while(rs_find.next()){
					if(rs_find.getString("SFDC_QuoteId") != null && !rs_find.getString("SFDC_QuoteId").isEmpty()){
			...
			result = "failed";
			logger.debug("UserSession.setSFDC_QuoteId()");
			logger.debug(e.getMessage());
			logger.debug("UserSession.setSFDC_QuoteId() END");
		}
		finally{
			try{
				stmt.close();
	...
	public String getDocPriorityName(){
		...
		try{
			...
			logger.debug(" QuoteHeaderBean.getDocPriorityName");
			logger.debug(e.getMessage());
			logger.debug(" QuoteHeaderBean.getDocPriorityNameEND");
		}
		finally{
			try{
				stmt.close();
	...
	public String getAssignedRepNo(){
		return assignedRepNo;
	}

	public String getSFDC_QuoteId(){
		return sfdc_QuoteId;
	...
	public String getXml(){
		...
		Statement stmt = null;
		try{
			//logger.debug(orderDate+":: getXml1");
			...
			Element templateElement = doc2.createElement("template");
			templateElement.appendChild(doc2.createTextNode(template.trim() + "#"));
			searchresult.appendChild(templateElement);
			...
			Element o = doc2.createElement("taxZip");
			o.appendChild(doc2.createTextNode(taxZip.trim() + "#"));
			searchresult.appendChild(o);
			Element p = doc2.createElement("taxExempt");
			p.appendChild(doc2.createTextNode(taxExempt.trim() + "#"));
			...
			myConn = DriverManager.getConnection("jdbc:jtds:sqlserver://" + dbServerName + ":" + dbServerPort + "/" + erapidDB,erapidDBUsername,erapidDBPassword);
			stmt = myConn.createStatement();
			//logger.debug(orderDate+":: getXml2");
			stmt.executeUpdate("set ANSI_warnings off");
			int notecounter = 0;
			...
			div = " error " + e + ":: " + query;
			logger.debug(" QuoteHeaderBean.getxml");
			logger.debug(e.getMessage());
			logger.debug(" QuoteHeaderBean.getxml END");
		}
		finally{
			try{
				stmt.close();
	...
	public String getPSAXml(String psaNo){
		...
		try{
			 ...
			 */
			Class.forName("net.sourceforge.jtds.jdbc.Driver");
			connPsa = DriverManager.getConnection("jdbc:jtds:sqlserver://" + dbPSAServerName + ":" + dbPSAServerPort + "/" + PSADB,PSADBUsername,PSADBPassword);
			...
			logger.debug("QuoteHeaderBean.getPSAXml");
			logger.debug(e.getMessage());
			logger.debug("QuoteHeaderBean.getPSAXml END");
		}
		finally{
			try{
				connPsa.close();
	...
	public String getCountries(){
		...
		try{
			...
			div = " error " + e + ":: " + query;
			logger.debug(" QuoteHeaderBean.getCountries");
			logger.debug(e.getMessage());
			logger.debug(" QuoteHeaderBean.getCountries END");
		}
		finally{
			try{
				stmt.close();
	...
	public String getPullDownValues(String pullDownCountry,String pullDownProductId,String pullDownGroupId,String pullDownSelectName){
		...
		try{
			...
			stmt = myConn.createStatement();
			stmt.executeUpdate("set ANSI_warnings off");
			//logger.debug(pullDownSelectName+"0");
			if(pullDownSelectName.toLowerCase().equals("bcc")){
				...
				//logger.debug("quoteheader bean line 1632::" + query);
				div = div + "<option value=''></option>";
				ResultSet rs1 = stmt.executeQuery(query);
				if(rs1 != null){
					while(rs1.next()){
			...
			//logger.debug(div + ":: pulldown query quoteheaderbean line 1428");
			myConn.close();
		}
		catch(Exception e){
			div = " error " + e + ":: " + query;
			logger.debug(" QuoteHeaderBean.getPullDownValues");
			logger.debug(e.getMessage());
			logger.debug(" QuoteHeaderBean.getPullDownValues END");
		}
		finally{
			try{
				stmt.close();
	...
	public String getPullDownValues2(String pullDownCountry,String pullDownProductId,String pullDownGroupId,String pullDownSelectName,String projectType,String quoteOrigin){
		...
		try{
			...
			if(pullDownSelectName.equals("emailQuotes")){
				...
				//logger.debug("quoteheader bean line 1798::" + query);
			...
			div = " error " + e + ":: " + query;
			logger.debug(" QuoteHeaderBean.getPullDownValues2");
			logger.debug(e.getMessage());
			logger.debug(" QuoteHeaderBean.getPullDownValues2 END");
		}
		finally{
			try{
				stmt.close();
	...
	public void setDefaultNotes(String notesProductId){
		...
		try{
			...
			stmt = myConn.createStatement();
			stmt.executeUpdate("set ANSI_warnings off");
			//logger.debug(exclusions+":: exc");
			//logger.debug(qualifyingNotes+":: qlf");
			...
			logger.debug(" QuoteHeaderBean.getDefaultNotes");
			logger.debug(e.getMessage());
			logger.debug(" QuoteHeaderBean.getDefaultNotes END");
		}
		finally{
			try{
				stmt.close();
	...
	public String getNotes(String notesProductId,String noteType){
		...
		try{
			...
			//logger.debug("select * from "+table_name+" where product_id='"+notesProductId+"' order by code");
			if(rs1 != null){
				while(rs1.next()){
			...
			logger.debug(" QuoteHeaderBean.getNotes");
			logger.debug(e.getMessage());
			logger.debug(" QuoteHeaderBean.getNotes END");
		}
		finally{
			try{
				stmt.close();
	...
	public String saveHeader(String orderNo,String projectName,String custName,String custLoc,String jobLoc,String projectState,String archName,String archLoc,String exclusions,String qualifyingNotes,String terriRep,String marketType,String projectState2,String internalNo,String winLoss,String docType,String docDate,String entryDate,String expiresDate,String winLossDesc,String exclusionsFreeText,String qualifyingNotesFreeText,String freeText,String internalNotes,String qtype,String product,String repNum,String altCpyNo,String custNo,String userId,String agentName,String projectType,String projectTypeId,String quoteType,String showRecap,String exchName,String exchRate,String exchDate,String groupCodes,String salesRegion,String constructionType,String endUser,String bpcsOrderNo,String pricingOptions,String pricingOptionsFree,String instructions,String docStage,String changeCustomer,String changeArch,String taxZip,String taxExempt){
		...
		//logger.debug(taxZip+"::"+taxExempt+":: quoteheaderbean line 2297");
		try{
			...
			if(orderNo.trim().length() > 0){
				try{
					...
					//logger.debug(oldType+":: old type");
					//logger.debug(docType+":: new type");
					//logger.debug(oldWinLoss+":: old status");
					//logger.debug(winLoss+":: new status");
					if(projectType.equals("SFDC") || projectType.equals("RP")){
						...
						logger.debug(sfdcStatus + "::" + orderNo + ":: sfdc status");
						if(serverName.indexOf("dev") > 0){
							logger.debug(" DEV SERVER--SFDC STATUS SCRIPT WONT RUN");
						}
						else{
							//logger.debug("run stored procedure");
							...
							if(sfdcStatus != null && sfdcStatus.trim().length() > 0){
								try{
									...
									cs.execute();
									cs.close();
									logger.debug("sfdc stored procedure run for " + orderNo + "::" + sfdcStatus);
								}
								catch(java.sql.SQLException e){
									logger.debug("Problem with salesforce stored procedure<br>");
									logger.debug("Illegal Operation try again/Report Error" + "<br>");
					...
					logger.debug(" QuoteHeaderBean.saveHeader");
					logger.debug(e.getMessage());
					logger.debug(" QuoteHeaderBean.saveHeader END");
				}

			}
			else{
				//logger.debug("quoteheaderbean line 1693 add new quote");
				//logger.debug("quote header bean line 2203" + qtype);
				//String bpcsOrderNo = "";
				...
				try{
					...
					//logger.debug(altCpyNo + "::1");
					ResultSet rsPFL = stmt.executeQuery("select template,quote_origin from cs_project where order_no='" + altCpyNo + "'");
					if(rsPFL != null){
						while(rsPFL.next()){
					...
					if(!(qtype.equals("alt") || qtype.equals("build") || qtype.equals("change") || qtype.equals("release") || qtype.equals("revision") || qtype.equals("submittal"))){
						...
						//logger.debug(altCpyNo + "::" + base + ":: quote header bean line 2050");
						String altNo = "";
					...
					//logger.debug(orderNo + ":: order number");

					if(orderNo != null && orderNo.trim().length() > 0){

						//logger.debug("quoteheaderbean line 1744 add cs_project");
						if(projectState == null || projectState.trim().length() == 0){
							projectState2 = projectState;
						}
						String georder = "";
						...
						insertCsProject.setString(43,taxZip);
						insertCsProject.setString(44,taxExempt);
						int rocount = insertCsProject.executeUpdate();
						insertCsProject.close();

						//logger.debug("quoteheaderbean line 1776 add cs_project end");
						//logger.debug(repNum+":: creator id");
						//logger.debug("quoteheaderbean line 1787 add doc_header" + "::" + "insert into doc_header (win_loss,doc_type,doc_date,entry_date,expires_date,win_loss_desc,doc_number,doc_revision,doc_customer,doc_salesperson) values (?,?,?,?,?,?,?,?,?,?)");
						java.sql.PreparedStatement insertDocHeader = myConn.prepareStatement("insert into doc_header (win_loss,doc_type,doc_date,entry_date,expires_date,win_loss_desc,doc_number,doc_revision,doc_customer,doc_salesperson,doc_priority,doc_stage) values (?,?,?,?,?,?,?,?,?,?,?,?)");
						insertDocHeader.setString(1,winLoss);
						insertDocHeader.setString(2,docType);
						insertDocHeader.setString(3,docDate);
						...
						int rocount2 = insertDocHeader.executeUpdate();
						insertDocHeader.close();
					//logger.debug("quoteheaderbean line 18101 add doc_header end::" + rocount2);
						//logger.debug(orderNo + ":: order number");

						//logger.debug("quoteheaderbean line 1970" + qtype);
						if(isSample || isProfileCopy || qtype.equals("alt") || qtype.equals("Alternate") || qtype.equals("cpy") || qtype.equals("build") || qtype.equals("change") || qtype.equals("release") || qtype.equals("revision") || qtype.equals("submittal")){
							//logger.debug("quote headerbean line 1972 copy lines");
			...
			logger.debug(" initialize db conn for new quote ");
			logger.debug(e.getMessage());
			logger.debug(" QuoteHeaderBean.saveHeader END");
		}
		finally{
	...
	private String copyLines(String oldOrderNo,String newOrderNo,String copyType,String newCreatorId,String dtype){
		//logger.debug("quoteheaderbean line 1330 copy lines");
		String tempQuery = "";
		Connection myConn = null;
		Statement stmt = null;
		Connection myConnSfdc = null;
		...
		try{
			...
			if(rsDm != null){
				while(rsDm.next()){
					logger.debug("copy dm records::" + rsDm.getString("table_name") + "::" + rsDm.getString("line_no_col") + "::" + rsDm.getString("order_no_col") + "::" + rsDm.getString("col_list") + "::");
			...
			for(int i = 0;i < tableNames.size();i++){
				...
				//logger.debug("quoteheaderbean line 1421::"+querySelect);
				//logger.debug("quoteheaderbean line 1422::"+queryInsert);
			...
			logger.debug(" QuoteHeaderBean.copyLines");
			logger.debug(e.getMessage());
			logger.debug(tempQuery);
			logger.debug(" QuoteHeaderBean.copyLines END");
		}
		finally{
			try{
				stmt.close();
	...
	public String deleteFactura(String orderNoFactura){
		...
		try{
			...
			logger.debug(" QuoteHeaderBean.deleteFactura");
			logger.debug(e.getMessage());
			logger.debug(" QuoteHeaderBean.deleteFActura END");
		}
		return value;
	}
	...
	public String checkSFDCType(String SFDCno){
		...
		try{
			...
			stmtSfdc.executeUpdate("set ANSI_warnings off");
			String temp = "";
			//logger.debug("3select RecordTypeId from Quotes__c where name='" + SFDCno + "'");
			...
			logger.debug("QuoteHeaderBean.checkSFDCType");
			logger.debug(e.getMessage());
			logger.debug("QuoteHeaderBean.checkSFDCType END");
		}
		finally{
			try{
				stmtSfdc.close();
	...
	public void setRepQuote(String orderNo,String repQuote){
		...
		try{
			...
			logger.debug("quoteHeaderBean.setRepQuote");
			logger.debug(e.getMessage());
			logger.debug("quoteHeaderBean.setRepQuote END");
		}
		finally{
			try{
				stmt.close();
	...
	public void setRep(String orderNo,String rep){
		Connection myConn = null;
		Statement stmt = null;
		try{
			//logger.debug(orderNo + "::" + rep + "::quoteheaderbean.java");
			File fXmlFile = new File("d:\\erapid\\erapid.xml");
			DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
			DocumentBuilder dBuilder = dbFactory.newDocumentBuilder();
			Document doc = dBuilder.parse(fXmlFile);
			...
			stmt = myConn.createStatement();
			stmt.executeUpdate("set ANSI_warnings off");
			int rs = stmt.executeUpdate("update cs_project set assigned_rep_no='" + rep + "',rep_quote='1' where order_no='" + orderNo + "'");
			myConn.close();
		}
		catch(Exception e){
			logger.debug("quoteHeaderBean.setRep");
			logger.debug(e.getMessage());
			logger.debug("quoteHeaderBean.setRep END");
		}
		finally{
			try{
				stmt.close();
	...
	public String getSalesRegion(String groupCode){
		...
		try{
			...
			logger.debug("quoteHeaderBean.getSalesRegion");
			logger.debug(e.getMessage());
			logger.debug("quoteHeaderBean.getSalesRegion END");
		}
		finally{
			try{
				stmt.close();
	...
	private String addQlyForModels(Statement stmt, String order_no, String existedQly) {
			
			String strReturn = 	existedQly;
		try{
			...
			String notes23 = "*RF-20*RV-20*VB640*VC1000*VC1100*VC1200*VC201*VC300*VC301*VC301RF*VC304*VC400*VC500*VC500M*VC6000*VC700*VC7315*VC8000*";
			...
			for(int i=0; i<distinctModels.size(); i++){
				...
				//out.println("Test "+distinctModels.elementAt(i).toString()+"::<BR>");
				
				if (notes23.indexOf("*" + distinctModels.elementAt(i).toString() + "*") >= 0) {
					qualifying_notes=qualifying_notes+"23,";
				//	qual_count++;
				}
			}
			...
			if(qualifying_notes != null&&qualifying_notes.trim().length()>0){
				strReturn =  existedQly + "," + qualifying_notes;
			}
			
			logger.debug("All qualifying_notes is " +strReturn);
		}
		catch(Exception ex){
			logger.debug("quoteHeaderBean.addQlyForModels");
			logger.debug(ex.getMessage());
			logger.debug("quoteHeaderBean.addQlyForModels END");
		}
			return strReturn;
		}

	// FOR TAC: # 73401
	private static boolean isGivenNumberExist(String strNumber, String qlyNotes) {
			
			boolean isExistAlready = false;
			
			if(null!=qlyNotes && !qlyNotes.isEmpty()) {
				
				String splitArray[] = qlyNotes.split(",");
				if(splitArray.length > 0) {
					
					for(String note: splitArray) {
						String qlfNoteCode = note;
						if(null!= qlfNoteCode && !qlfNoteCode.isEmpty()) {
							qlfNoteCode = qlfNoteCode.trim();
							if(qlfNoteCode.length()==2) {
								isExistAlready = qlfNoteCode.equalsIgnoreCase(strNumber);
								if(isExistAlready) {
									break;
								}
							}
